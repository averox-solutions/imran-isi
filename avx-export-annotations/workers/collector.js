...
/** Export shared notes via avx-pads in the desired format
...
async function collectSharedNotes(retries = 3) {
  ...
  const notesEndpoint = `${config.avxPadsAPI}/p/${padId}/export/${notesFormat}`;
  ...
  try {
    ...
    if (retries > 0 && err?.response?.status == 429) {
      // Wait for the avx-pads API to be available due to rate limiting
