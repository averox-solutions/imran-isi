...
class App extends React.Component {
  ...
  fetchMostUsedEmojis() {
    ...
    emojis.forEach((emoji) => {
      icons[emoji] = (<i className={`${emojiConfigs[emoji].icon} avx-icon-card`} />);
    });
  ...
  fetchActivitiesJson() {
    ...
    if (learningDashboardAccessToken !== '') {
      ...
      const url = new URL('/averox/api/learningDashboard', window.location);
  ...
  render() {
    ...
    document.title = `${intl.formatMessage({ id: 'app.learningDashboard.averoxTitle', defaultMessage: 'Averox' })} - ${intl.formatMessage({ id: 'app.learningDashboard.dashboardTitle', defaultMessage: 'Learning Analytics Dashboard' })} - ${activitiesJson.name}`;
