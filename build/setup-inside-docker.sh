...
VERSION_NUMBER="$(cat "$SOURCE/averox-config/averox-release" | cut -d '=' -f2 | cut -d "-" -f1)"
# this contains stuff like alpha4 etc
VERSION_ADDON="$(cat "$SOURCE/averox-config/averox-release" | cut -d '=' -f2 | cut -d "-" -f2)"
...
build_package() {
    ...
    # some packages do not need source files from the repo (e.g. the averox meta-package)
