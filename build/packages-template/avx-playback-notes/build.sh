...
chmod +x staging/usr/local/averox/core/scripts/process/notes.rb
chmod +x staging/usr/local/averox/core/scripts/publish/notes.rb

mkdir -p staging/usr/share/averox/nginx
cp scripts/notes-playback.nginx staging/usr/share/averox/nginx
...
fpm -s dir -C ./staging -n $PACKAGE \
    ...
    --description "Averox notes recording format" \
    $DIRECTORIES \
    $OPTS
