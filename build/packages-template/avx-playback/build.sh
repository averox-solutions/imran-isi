...
BBB_PLAYBACK_BASE=staging/var/averox/$BBB_PLAYBACK_HOMEPAGE
...
grep \"version\"\: package.json | sed -e 's|.*\ \"||g' -e 's|\".*||g' > avx-playback-version
mkdir -p $BBB_PLAYBACK
cp -r ./build/* avx-playback-version $BBB_PLAYBACK


mkdir -p staging/usr/share/averox/nginx
cp playback.nginx staging/usr/share/averox/nginx
...
fpm -s dir -C ./staging -n $PACKAGE \
    ...
    --description "Player for Averox presentation format recordings" \
    $DIRECTORIES \
    $OPTS
