...
cd $BUILDDIR
	...
	rm -rf $CONFDIR/*
	cp -r avx-voice-conference/config/freeswitch/conf/* $CONFDIR
...
HERE
  ...
  mkdir -p $DESTDIR/usr/local/bin
	cp fs_cliavx $DESTDIR/usr/local/bin
	chmod +x $DESTDIR/usr/local/bin/fs_cliavx
...
fpm -s dir -C $DESTDIR -n $PACKAGE \
    ...
    --description "Averox build of FreeSWITCH" \
    $DIRECTORIES                            \
    $OPTS
