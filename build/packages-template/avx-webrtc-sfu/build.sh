...
DIRS="/usr/share/averox/nginx /usr/local/averox/avx-webrtc-sfu /etc/logrotate.d /usr/lib/systemd/system /etc/cron.hourly"
for dir in $DIRS; do
  mkdir -p staging$dir
done
...
mkdir -p staging/usr/local/averox/avx-webrtc-sfu

find -maxdepth 1 ! -path . ! -name staging $(printf "! -name %s " $(cat .build-files)) -exec cp -r {} staging/usr/local/averox/avx-webrtc-sfu/ \;

pushd .
cd staging/usr/local/averox/avx-webrtc-sfu/
...
cp webrtc-sfu.nginx staging/usr/share/averox/nginx

cp avx-webrtc-sfu.service staging/usr/lib/systemd/system
cp avx-webrtc-sfu.logrotate staging/etc/logrotate.d
rm -rf staging/usr/local/averox/avx-webrtc-sfu/.git
