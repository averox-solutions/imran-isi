...
# as of March 12, 2022, circa Averox 2.5-alpha4, we set npm by default to 8.5.0
# however, it seems avx-etherpad has troubles building with npm as high.
# Setting npm to 6.14.11 which was used successfully for building in Averox 2.4.x
npm -v
npm i -g npm@6.14.11
npm -v
...
rm -rf avx-etherpad-plugin
git clone https://github.com/alangecker/avx-etherpad-plugin.git
npm pack ./avx-etherpad-plugin
npm install ./ep_averox_patches-*.tgz
...
git clone https://github.com/alangecker/avx-etherpad-skin.git staging/usr/share/etherpad-lite/src/static/skins/averox
...
cp notes.nginx staging/usr/share/averox/nginx

rm -rf staging/usr/share/etherpad-lite/src/static/skins/averox/.git
...
fpm -s dir -C ./staging -n $PACKAGE \
    ...
    --description "The EtherPad Lite components for Averox" \
    $DIRECTORIES \
    $OPTS \
