...
cp -r playback staging/usr/local/averox/core

mkdir -p staging/usr/share/averox/nginx
mv staging/usr/local/averox/core/scripts/playback-video.nginx staging/usr/share/averox/nginx
...
fpm -s dir -C ./staging -n $PACKAGE \
    ...
    --description "Averox video recording format" \
    $DIRECTORIES \
    $OPTS
