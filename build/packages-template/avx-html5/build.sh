...
cp avx-html5.nginx staging/usr/share/averox/nginx

mkdir -p staging/etc/nginx/conf.d
cp avx-html5-conn-limit.conf staging/etc/nginx/conf.d
...
# only package.json. The dependencies for avx-html5 are already installed in
...
cp avx-html5.service staging/usr/lib/systemd/system
...
cp averox.nginx staging/etc/nginx/sites-available/averox

mkdir -p staging/usr/share/averox/nginx
cp sip.nginx staging/usr/share/averox/nginx

mkdir -p staging/var/www/averox
touch staging/var/www/averox/index.html
...
fpm -s dir -C ./staging -n $PACKAGE \
    ...
    --description "The HTML5 components for Averox" \
    $DIRECTORIES \
    $OPTS
