...
sed -i "s|\(version := \)\".*|\1\"$EPHEMERAL_VERSION\"|g" avx-fsesl-client/build.sbt
find -name build.gradle -exec sed -i "s|\(.*org.averox.*avx-fsesl-client[^:]*\):.*|\1:$EPHEMERAL_VERSION'|g" {} \;
find -name build.sbt -exec sed -i "s|\(.*org.averox.*avx-fsesl-client[^\"]*\"[ ]*%[ ]*\)\"[^\"]*\"\(.*\)|\1\"$EPHEMERAL_VERSION\"\2|g" {} \;
...
sed -i 's|\(val avxCommons = \)"[^"]*"$|\1"EPHEMERAL_VERSION"|g' project/Dependencies.scala
sed -i 's|\(val avxFsesl = \)"[^"]*"$|\1"EPHEMERAL_VERSION"|g' project/Dependencies.scala
...
echo '#JAVA_OPTS="-Dconfig.file=/usr/share/avx-fsesl-akka/conf/application.conf $JAVA_OPTS"' > src/templates/etc-default
