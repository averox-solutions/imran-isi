package org.averox.endpoint.redis

import org.apache.pekko.actor.{Actor, ActorLogging, ActorSystem, Props}
import org.averox.common2.msgs._
import org.averox.core.OutMessageGateway
import org.averox.core.api.{UserClosedAllGraphqlConnectionsInternalMsg, UserEstablishedGraphqlConnectionInternalMsg}
import org.averox.core.bus.{AveroxEvent, InternalEventBus}
import org.averox.core.db.UserGraphqlConnectionDAO
...
class GraphqlConnectionsActor(
  ...
  private def handleBbbCommonEnvCoreMsg(msg: BbbCommonEnvCoreMsg): Unit = {
    msg.core match {
      ...
      // Messages from avx-graphql-middleware
  ...
  private def handleUserGraphqlConnectionEstablishedSysMsg(msg: UserGraphqlConnectionEstablishedSysMsg): Unit = {
    ...
    for {
      ...
      //Send internal message informing user has connected
        eventBus.publish(AveroxEvent(user.meetingId,
  ...
  private def handleUserGraphqlConnectionClosed(sessionToken: String, middlewareUID: String, browserConnectionId: String): Unit = {
    ...
    for {
      ...
      if (!graphqlConnections.values.exists(c => c.sessionToken == sessionToken)) {
        eventBus.publish(AveroxEvent(user.meetingId, UserClosedAllGraphqlConnectionsInternalMsg(user.intId)))
      }
    }
  }
