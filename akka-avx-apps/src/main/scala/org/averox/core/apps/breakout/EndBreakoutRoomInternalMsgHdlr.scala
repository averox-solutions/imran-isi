...
import org.averox.common2.msgs.{ BbbClientMsgHeader, BbbCommonEnvCoreMsg, BbbCoreEnvelope, BbbCoreHeaderWithMeetingId, ExportJob, MessageTypes, PresentationConversionUpdateEvtMsg, PresentationConversionUpdateEvtMsgBody, PresentationConversionUpdateSysPubMsg, PresentationPageForExport, PresentationUploadTokenSysPubMsg, PresentationUploadTokenSysPubMsgBody, Routing, StoreExportJobInRedisSysMsg, StoreExportJobInRedisSysMsgBody, StoredAnnotations }
import org.averox.core.api.{ CapturePresentationReqInternalMsg, EndBreakoutRoomInternalMsg }
import org.averox.core.apps.presentationpod.PresentationPodsApp
import org.averox.core.bus.{ AveroxEvent, InternalEventBus }
import org.averox.core.db.{ PresPresentationDAO }
import org.averox.core.models.{ Pads, PresentationInPod, PresentationPage, PresentationPod }
...
trait EndBreakoutRoomInternalMsgHdlr extends HandlerHelpers {
  ...
  def handleEndBreakoutRoomInternalMsg(msg: EndBreakoutRoomInternalMsg): Unit = {
    if (liveMeeting.props.breakoutProps.captureSlides) {
      ...
      val captureSlidesEvent = AveroxEvent(msg.breakoutId, CapturePresentationReqInternalMsg("system", msg.parentId, filename))
