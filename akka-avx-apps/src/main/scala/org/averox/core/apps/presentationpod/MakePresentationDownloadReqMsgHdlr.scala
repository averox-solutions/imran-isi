...
import org.averox.core.api.{ CapturePresentationReqInternalMsg }
...
import org.averox.core.db.{ ChatMessageDAO, PresPresentationDAO }
import org.averox.core.domain.MeetingState2x
import org.averox.core.running.LiveMeeting
import org.averox.core.util.RandomStringGenerator
import org.averox.core.models.{ PresentationInPod, PresentationPage, PresentationPod }
...
trait MakePresentationDownloadReqMsgHdlr extends RightsManagementTrait {
  ...
  def handle(m: MakePresentationDownloadReqMsg, state: MeetingState2x, liveMeeting: LiveMeeting, bus: MessageBus): Unit = {
    ...
    if (liveMeeting.props.meetingProp.disabledFeatures.contains("downloadPresentationWithAnnotations")
      ...
      val presLocation = List("var", "averox", meetingId, meetingId, presId).mkString(File.separator, File.separator, "");
  ...
  def handle(m: CapturePresentationReqInternalMsg, state: MeetingState2x, liveMeeting: LiveMeeting, bus: MessageBus): Unit = {
    ...
    // Informs avx-web about the token so that when we use it to upload the presentation, it is able to look it up in the list of tokens
