...
class MeetingActorAudit(
  ...
  def handleMonitorNumberOfWebUsers() {
    eventBus.publish(AveroxEvent(props.meetingProp.intId, MonitorNumberOfUsersInternalMsg(props.meetingProp.intId)))
    eventBus.publish(AveroxEvent(props.meetingProp.intId, MonitorGuestWaitPresenceInternalMsg(props.meetingProp.intId)))

    // Trigger updating users of time remaining on meeting.
    eventBus.publish(AveroxEvent(props.meetingProp.intId, SendTimeRemainingAuditInternalMsg(props.meetingProp.intId, 0)))
    ...
    if (props.recordProp.record) {
      eventBus.publish(AveroxEvent(props.meetingProp.intId, SendRecordingTimerInternalMsg(props.meetingProp.intId)))
    }
  }
