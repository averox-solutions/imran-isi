...
import org.averox.core.api.{BreakoutRoomEndedInternalMsg, DestroyMeetingInternalMsg, EndBreakoutRoomInternalMsg}
...
import org.averox.core.db.{BreakoutRoomUserDAO, MeetingDAO, MeetingRecordingDAO, NotificationDAO, UserBreakoutRoomDAO}
...
import org.averox.core2.message.senders.{MsgBuilder, UserJoinedMeetingEvtMsgBuilder}
...
trait HandlerHelpers extends SystemConfiguration {
  ...
  def destroyMeeting(eventBus: InternalEventBus, meetingId: String): Unit = {
    eventBus.publish(AveroxEvent(meetingManagerChannel, new DestroyMeetingInternalMsg(meetingId)))
  }
  ...
  def endAllBreakoutRooms(eventBus: InternalEventBus, liveMeeting: LiveMeeting, state: MeetingState2x, reason: String): MeetingState2x = {
    for {
      ...
      model.rooms.values.foreach { room =>
        eventBus.publish(AveroxEvent(room.id, EndBreakoutRoomInternalMsg(liveMeeting.props.meetingProp.intId, room.id, reason)))
