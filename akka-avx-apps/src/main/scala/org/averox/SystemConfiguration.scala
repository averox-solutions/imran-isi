...
trait SystemConfiguration {
  ...
  lazy val avxWebHost = Try(config.getString("services.avxWebHost")).getOrElse("localhost")
  lazy val avxWebPort = Try(config.getInt("services.avxWebPort")).getOrElse(8888)
  lazy val avxWebAPI = Try(config.getString("services.avxWebAPI")).getOrElse("localhost")
  lazy val avxWebSharedSecret = Try(config.getString("services.sharedSecret")).getOrElse("changeme")
  lazy val checkSumAlgorithmForBreakouts = Try(config.getString("services.checkSumAlgorithmForBreakouts")).getOrElse("sha256")
  lazy val avxWebModeratorPassword = Try(config.getString("services.moderatorPassword")).getOrElse("changeme")
  lazy val avxWebViewerPassword = Try(config.getString("services.viewerPassword")).getOrElse("changeme")
  ...
  lazy val fromBbbWebRedisChannel = Try(config.getString("redis.fromBbbWebRedisChannel")).getOrElse("from-avx-web-redis-channel")
