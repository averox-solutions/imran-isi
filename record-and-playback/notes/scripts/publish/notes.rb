...
avx_props = YAML::load(File.open('../../core/scripts/averox.yml'))
  ...
  if (playback == "notes")
    ...
    if not FileTest.directory?(target_dir)
      ...
      if File.exist? note_file
        Averox.logger.info("Copying: #{note_file} to -> #{target_dir}")
        ...
        Averox.add_raw_size_to_metadata(target_dir, raw_dir)
        Averox.add_playback_size_to_metadata(target_dir)

        FileUtils.cp_r(target_dir, publish_dir) # Copy all the files.
        Averox.logger.info("Finished publishing script notes.rb successfully.")
      else
        Averox.logger.info("There wasn't any note for #{meeting_id}")
      end

      Averox.logger.info("Removing processed files.")
      FileUtils.rm_r(process_dir)

      Averox.logger.info("Removing published files.")
