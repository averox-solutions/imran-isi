...
#
# This file is part of the Averox open source conferencing system.
#
...
props = File.open(File.expand_path('averox.yml', __dir__)) do |avx_yml|
  YAML.safe_load(avx_yml)
end

logger = Journald::Logger.new('avx-rap-caption-inbox')
Averox.logger = logger

captions_dir = props['captions_dir']
unless captions_dir
  logger.error('captions_dir was not defined in averox.yml')
  exit(1)
end
...
caption_file_notify = proc do |json_filename|
  ...
  logger.tag(record_id: record_id) do
    begin
      ...
      ret = Averox.exec_ret(*ffmpeg_cmd)
      ...
      Dir.glob(File.expand_path('captions/*', __dir__)) do |caption_script|
        ...
        ret = Averox.exec_ret(caption_script, '--record-id', record_id)
