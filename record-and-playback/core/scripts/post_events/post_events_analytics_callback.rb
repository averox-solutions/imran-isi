...
# in addition to the ones included with Averox:
# * avxevents
# * jwt
#
# Running post_events scripts requires that you enable keepEvents=true in
# avx-web's averox.properties configuration.
...
file = File.open('/var/log/averox/post_events_analytics_callback.log', File::WRONLY | File::APPEND | File::CREAT)
...
props = YAML.safe_load(File.open('../../core/scripts/averox.yml'))
...
def send_data(analytics_url, secret, payload)
  ...
  request = Net::HTTP::Post.new(uri.request_uri, {
    ...
    'User-Agent' => 'Averox Analytics Callback'
  })
  ...
  Averox.logger.info(response.body)

  if code < 200 || code >= 300
    Averox.logger.info("Failed when calling #{uri.request_uri}")
  else
    Averox.logger.info("Success")
  end
end
...
def format_analytics_data!(data)
  ...
  # Remove the internal meeting id generated by avxevents gem
  data = data.tap { |hs|
    hs.delete("meeting_id")
  }

  # Convert CamelCase keys to snake_keys. This is done in avxevents gem
...
# Main
#
Averox.logger.info("Analytics Post Events for [#{meeting_id}] starts")
...
begin
  ...
  unless analytics_callback_url.nil?
    Averox.logger.info("Processing events for analytics...")
    filepathOverride = "/etc/averox/avx-web.properties"
    hasOverride = File.file?(filepathOverride)

    avx_props = JavaProperties::Properties.new("/usr/share/avx-web/WEB-INF/classes/averox.properties")
    
    # If the file does exists: 
    if (hasOverride)
      avxOverrideProps = JavaProperties::Properties.new(filepathOverride)
      # Override the props
      avxOverrideProps.each do |key, prop|
        avx_props[key]=prop
      end
    end

    secret = avx_props[:securitySalt]
    ...
    Averox.logger.info(payload.to_json)
...
Averox.logger.info("Analytics Post Events for [#{meeting_id}] ends")
