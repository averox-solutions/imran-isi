...
def archive_events(meeting_id, redis_host, redis_port, redis_password, raw_archive_dir, break_timestamp)
  Averox.logger.info("Archiving events for #{meeting_id}")
  redis = Averox::RedisWrapper.new(redis_host, redis_port, redis_password)
  events_archiver = Averox::RedisEventsArchiver.new(redis)
...
def archive_notes(meeting_id, notes_endpoint, notes_formats, raw_archive_dir)
  Averox.logger.info("Archiving notes for #{meeting_id}")
  events = Nokogiri::XML(File.open("#{raw_archive_dir}/#{meeting_id}/events.xml"))
  notes_id = Averox::Events.get_notes_id(events)
  ...
  if base_path_notes != "undefined"
    Averox.try_download("#{notes_endpoint}/#{base_path_notes}/export/txt", tmp_note)
  end
  if File.exist? tmp_note
    ...
    if blank
      Averox.logger.info("Empty notes for #{meeting_id}")
      return
    end
  else
    Averox.logger.info("Notes were not used in #{meeting_id}")
    return
  end

  notes_formats.each do |format|
    Averox.try_download("#{notes_endpoint}/#{CGI.escape notes_id}/export/#{format}", "#{notes_dir}/notes.#{format}")
  end
end

def archive_audio(meeting_id, audio_dir, raw_archive_dir)
  Averox.logger.info("Archiving audio #{audio_dir}/#{meeting_id}-*.*")
  ...
  if audio_files.empty?
    Averox.logger.warn("No audio found for #{meeting_id}")
    return
  end
  audio_files.each do |audio_file|
    ...
    if audio_file.end_with?('.wav')
      ...
      Averox::EDL.encode(audio_file, nil, AUDIO_ARCHIVE_FORMAT, output_basename)
    else
      ret = Averox.exec_ret('rsync', '-stv', audio_file, audio_dest_dir)
      Averox.logger.warn("Failed to archive #{audio_file}") if ret != 0
    end
  end
end

def delete_audio(meeting_id, audio_dir)
  Averox.logger.info("Deleting audio #{audio_dir}/#{meeting_id}-*.*")
...
def remux_and_archive(source_dir, dest_dir)
  ...
  if files.empty?
    Averox.logger.warn("No media files found in #{source_dir}")
    return
  end

  Averox.logger.info("Remuxing and archiving files at #{source_dir}")
  ...
  files .each do |file|
    ...
    Averox.logger.warn("Failed to remux #{file}, archiving anyways: #{e}")
...
def archive_directory(source, dest)
  Averox.logger.info("Archiving contents of #{source}")
  FileUtils.mkdir_p(dest)
  ret = Averox.exec_ret('rsync', '-rstv',
          "#{source}/", "#{dest}/")
  if ret != 0
    Averox.logger.warn("Failed to archive contents of #{source}")
  end
end

def archive_has_recording_marks?(meeting_id, raw_archive_dir, break_timestamp)
  Averox.logger.info("Fetching the recording marks for #{meeting_id}.")
  ...
  start_timestamp = Averox::Events.get_segment_start_timestamp(
          doc, break_timestamp)
  end_timestamp = Averox::Events.get_segment_end_timestamp(
          doc, break_timestamp)
  Averox.logger.info("Segment start: #{start_timestamp}, end: #{end_timestamp}")

  Averox.logger.info("Checking for recording marks for #{meeting_id} segment #{break_timestamp}")
  rec_events = Averox::Events.match_start_and_stop_rec_events(
          Averox::Events.get_start_and_stop_rec_events(doc, true))
  ...
  Averox.logger.info("Recording marks found: #{has_recording_marks}")
  has_recording_marks
end
...
Averox.logger = Logger.new("#{log_dir}/archive-#{meeting_id}.log", 'daily' )
...
if break_timestamp.nil?
  Averox.logger.info('Deleting originals of archived media files.')
  # FreeSWITCH Audio files
...
if not archive_has_recording_marks?(meeting_id, raw_archive_dir, break_timestamp)
  Averox.logger.info("There's no recording marks for #{meeting_id}, not processing recording.")

  if break_timestamp.nil?
    # we need to delete the keys here because the sanity phase might not
    # automatically happen for this recording
    Averox.logger.info("Deleting redis keys")
    redis = Averox::RedisWrapper.new(redis_host, redis_port, redis_password)
    events_archiver = Averox::RedisEventsArchiver.new(redis)
