...
def parse_meeting_id(done_file)
  ...
  if (match = /^([0-9a-f]+-[0-9]+)$/.match(id))
    ...
    Averox.logger.warn("Recording done file #{done_file} has invalid format")
    nil
  end
end
...
def keep_meeting_events(recording_dir, file, from_archive)
  ...
  if from_archive
    ...
    Averox.logger.info("Enqueueing job to keep events from archive #{attrs.inspect}")
    Resque.enqueue(Averox::Resque::EventsWorker, attrs)
  else
    ...
    Averox.logger.info("Enqueueing job to keep events #{attrs.inspect}")
    Resque.enqueue(Averox::Resque::EventsWorker, attrs)
  end
end
...
def archive_recorded_meetings(recording_dir, done_file)
  id = File.basename(done_file, '.done')
  Averox.logger.debug("Seen new recorded done file for #{id}")
  ...
  Averox.logger.info("Enqueuing job to archive #{attrs.inspect}")
  Resque.enqueue(Averox::Resque::ArchiveWorker, attrs)
...
begin
  ...
  Averox.logger.debug('Running rap-starter...')
  ...
  Averox.logger.info("Setting up inotify watch on #{ended_status_dir}")
  ...
  Averox.logger.info("Setting up inotify watch on #{archived_status_dir}")
  ...
  Averox.logger.info("Setting up inotify watch on #{recorded_status_dir}")
  ...
  Averox.logger.info('Waiting for new recordings...')
