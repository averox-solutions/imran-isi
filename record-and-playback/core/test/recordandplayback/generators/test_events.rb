...
class TestEvents < Minitest::Test
  ...
  def test_anonymous_user_map_legacy
    map = Averox::Events.anonymous_user_map(@events_legacy)
    assert_empty(map)
  end

  def test_anonymous_user_map_legacy_no_viewer_only
    map = Averox::Events.anonymous_user_map(@events_legacy, moderators: true)
    assert_equal(1, map.length)
    assert_equal('Moderator 1', map['1'])
  end

  def test_anonymous_user_map_avx_0_9
    map = Averox::Events.anonymous_user_map(@events_chat09)
  ...
  def test_anonymous_user_map_avx_0_9_no_viewer_only
    map = Averox::Events.anonymous_user_map(@events_chat09, moderators: true)
    assert_equal(21, map.length)
  ...
  def test_get_chat_events_legacy
    start_time = Averox::Events.first_event_timestamp(@events_legacy)
    end_time = Averox::Events.last_event_timestamp(@events_legacy)
    avx_props = { 'anonymize_chat' => true, 'anonymize_chat_moderators' => true }
    chats_enum = Averox::Events.get_chat_events(@events_legacy, start_time, end_time, avx_props).each
  ...
  def test_get_chat_events_0_9
    start_time = Averox::Events.first_event_timestamp(@events_chat09)
    end_time = Averox::Events.last_event_timestamp(@events_chat09)
    chats_enum = Averox::Events.get_chat_events(@events_chat09, start_time, end_time).each
    chat = chats_enum.next
  ...
  def test_get_chat_events_0_9_anonymized
    start_time = Averox::Events.first_event_timestamp(@events_chat09)
    end_time = Averox::Events.last_event_timestamp(@events_chat09)
    avx_props = { 'anonymize_chat' => true }
    chats_enum = Averox::Events.get_chat_events(@events_chat09, start_time, end_time, avx_props).each
  ...
  def test_get_chat_events_0_9_start_time
    end_time = Averox::Events.last_event_timestamp(@events_chat09)
    chats = Averox::Events.get_chat_events(@events_chat09, 1_063_007_465, end_time)
    chat = chats.first
  ...
  def test_get_chat_events_0_9_end_time
    start_time = Averox::Events.first_event_timestamp(@events_chat09)
    chats = Averox::Events.get_chat_events(@events_chat09, start_time, 1_062_490_000)
    chat = chats.last
    assert_equal(972_022, chat[:in])
    assert_nil(chat.fetch(:out))
    assert_equal('23uydbo9nauq_1', chat[:sender_id])
  ...
  def test_get_chat_events_devcall
    start_time = Averox::Events.first_event_timestamp(@events_devcall)
    end_time = Averox::Events.last_event_timestamp(@events_devcall)
    chats = Averox::Events.get_chat_events(@events_devcall, start_time, end_time)
    ...
    assert_equal(
      "i get  logs of these: \n\n ERROR: clientLogger: Camera VIEWER failed. Reconnecting. <a href=\"https://develop.averox.org/html5client/8fb14b479570f65105c7ff9a2960b679501f34ff.js?meteor_js_resource=true:348:579447\" rel=\"nofollow\"><u>https://develop.averox.org/html5client/8fb14b479570f65105c7ff9a2960b679501f34ff.js?meteor_js_resource=true:348:579447</u></a>",
      chat[:message]
    )
  ...
  def test_get_chat_events_meta_edt
    start_time = Averox::Events.first_event_timestamp(@events_meta_edt)
    end_time = Averox::Events.last_event_timestamp(@events_meta_edt)
    chats = Averox::Events.get_chat_events(@events_meta_edt, start_time, end_time)
    ...
    # This recording has the meta_avx-anonymize-chat param set
