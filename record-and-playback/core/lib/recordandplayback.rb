...
module Averox
  ...
  # Averox logs information about its progress.
  ...
  def self.logger
    ...
    logger = Journald::Logger.new('avx-rap')
  ...
  def self.execute(command, fail_on_error = true)
    Averox.logger.info("Executing: #{command.respond_to?(:to_ary) ? Shellwords.join(command) : command}")
    ...
    Averox.logger.info("Success?: #{status.success?}")
    Averox.logger.info("Process exited? #{status.exited?}")
    Averox.logger.info("Exit status: #{status.exitstatus}")
  ...
  def self.exec_redirect_ret(outio, *command)
    Averox.logger.info "Executing: #{Shellwords.join(command)}"
    Averox.logger.info "Sending output to #{outio}"
    IO.pipe do |r, w|
      ...
      Process.waitpid(pid)
      Averox.logger.info "Exit status: #{$?.exitstatus}"
      return $?.exitstatus
    end
  end
  ...
  def self.download(url, output)
    Averox.logger.info "Downloading #{url} to #{output}"
  ...
  def self.try_download(url, output)
    begin
      ...
      Averox.logger.error "Failed to download file: #{e.to_s}"
  ...
  def self.add_raw_size_to_metadata(dir_name, raw_dir_name)
    size = Averox.get_dir_size(raw_dir_name)
    Averox.add_tag_to_xml("#{dir_name}/metadata.xml", "//recording", "raw_size", size)
  end

  def self.add_playback_size_to_metadata(dir_name)
    size = Averox.get_dir_size(dir_name)
    Averox.add_tag_to_xml("#{dir_name}/metadata.xml", "//recording/playback", "size", size)
  end

  def self.add_download_size_to_metadata(dir_name)
    size = Averox.get_dir_size(dir_name)
    Averox.add_tag_to_xml("#{dir_name}/metadata.xml", "//recording/download", "size", size)
  end
  ...
  def self.done_to_timestamp(r)
    Averox.record_id_to_timestamp(File.basename(r, ".done"))
  end
  ...
  def self.rap_scripts_path
    File.join(Averox.rap_core_path, 'scripts')
  end

  def self.read_props
    return @props if @props

    filepathRecOverride = "/etc/averox/recording/recording.yml"
    hasOverride = File.file?(filepathRecOverride)
    
    filepath = File.join(Averox.rap_scripts_path, 'averox.yml')
