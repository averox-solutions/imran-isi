    ...
    module Audio
      ...
      def self.dump(edl)
        ...
        edl.each do |entry|
          ...
          Averox.logger.debug "  Audio:"
          audio = entry[:audio]
          if audio
            Averox.logger.debug "    #{audio[:filename]} at #{audio[:timestamp]}"
          else
            Averox.logger.debug "    silence"
          end
        end
      end

      def self.mixer(inputs, output_basename)
        Averox.logger.debug "Mixing audio files"
        ...
        Averox.logger.info "Running audio mixer..."
      ...
      def self.render(edl, output_basename)
        ...
        Averox.logger.info "Reading source audio information"
        audioinfo.keys.each do |audiofile|
          Averox.logger.debug "  #{audiofile}"
          info = audio_info(audiofile)
          if !info[:audio] || !info[:duration]
            Averox.logger.warn "    This audio file is corrupt! It will be removed from the output."
            corrupt_audios << audiofile
            next
          end

          Averox.logger.debug "    format: #{info[:format][:format_name]}, codec: #{info[:audio][:codec_name]}"
          Averox.logger.debug "    sample rate: #{info[:sample_rate]}, duration: #{info[:duration]}"

          audioinfo[audiofile] = info
        end

        if corrupt_audios.length > 0
          Averox.logger.info "Removing corrupt audio files from EDL"
          edl.each do |event|
        ...
        ffmpeg_inputs = []
        ffmpeg_filter = ''
        Averox.logger.info "Generating ffmpeg command"
        for i in 0...(edl.length - 1)
          entry = edl[i]
          audio = entry[:audio]
          ...
          if audio
            Averox.logger.info "  Using input #{audio[:filename]}"

            speed = 1
            seek = audio[:timestamp]

            # Check for and handle audio files with mismatched lengths (generated
            # by buggy versions of freeswitch in old Averox
            if ((audioinfo[audio[:filename]][:format][:format_name] == 'wav' ||
              ...
              seek = 0
              Averox.logger.warn "  Audio file length mismatch, adjusting speed to #{speed}"
            end
            ...
            Averox.logger.info "  Generating silence"
        ...
        Averox.logger.info "Running audio processing..."
        exitstatus = Averox.exec_ret(*ffmpeg_cmd)
        raise "ffmpeg failed, exit code #{exitstatus}" if exitstatus != 0
      ...
      def self.audio_info(filename)
        IO.popen([*FFPROBE, filename]) do |probe|
          ...
          begin
            ...
            Averox.logger.warn("Couldn't parse audio info: #{e}")
          end
          return {} if !info
          ...
          if info[:format][:format_name] == 'wav'
            ...
            Averox.logger.debug("Recalculated duration from wav file length")
