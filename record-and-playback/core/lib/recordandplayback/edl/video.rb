...
module Averox
  module EDL
    module Video
      ...
      def self.dump(edl)
        Averox.logger.debug "EDL Dump:"
        edl.each do |entry|
          Averox.logger.debug "---"
          Averox.logger.debug "  Timestamp: #{entry[:timestamp]}"
          Averox.logger.debug "  Video Areas:"
          entry[:areas].each do |name, videos|
            Averox.logger.debug "    #{name}"
            videos.each do |video|
              Averox.logger.debug "      #{video[:filename]} at #{video[:timestamp]} (original duration: #{video[:original_duration]})"
            end
          end
        end
      end
      ...
      def self.enforce_cut_lengths(edl, framerate)
        ...
        (edl.length - 1).downto(1).each do |i|
          ...
          Averox.logger.debug("Dropping EDL entry index #{i - 1} (#{duration} < #{min_cut_len})")
          edl.delete_at(i - 1)
        ...
        if edl[0][:timestamp] != 0
          ...
          Averox.logger.debug('Resetting timestamp of first EDL entry to 0')
        ...
        if edl.length == 1
          Averox.logger.debug('EDL contains no cuts - enforcing minimum length')
      ...
      def self.render(edl, layout, output_basename)
        ...
        Averox.logger.info "Pre-processing EDL"
        ...
        Averox.logger.info "Reading source video information"
        videoinfo.keys.each do |videofile|
          Averox.logger.debug "  #{videofile}"
          info = video_info(videofile)
          if !info[:video]
            Averox.logger.warn "    This video file is corrupt! It will be removed from the output."
            corrupt_videos << videofile
          else
            Averox.logger.debug "    width: #{info[:width]}, height: #{info[:height]}, duration: #{info[:duration]}, start_time: #{info[:start_time]}"
            if info[:video][:deskshare_timestamp_bug]
              Averox.logger.debug("    has early 1.1 deskshare timestamp bug")
            elsif info[:format][:format_name] == 'flv' and info[:start_time] > 1
              Averox.logger.debug("    has large start time, needs remuxing")
        ...
        if remux_flv_videos.length > 0
          Averox.logger.info("Remuxing flv files with large start time")
          remux_flv_videos.each do |videofile|
            Averox.logger.info("  #{File.basename(videofile)}")
            ...
            if !info[:video]
              Averox.logger.warn("    Result of remux is corrupt, not using it.")
              next
            end
            Averox.logger.debug "    width: #{info[:width]}, height: #{info[:height]}, duration: #{info[:duration]}, start_time: #{info[:start_time]}"
        ...
        if corrupt_videos.length > 0
          Averox.logger.info "Removing corrupt video files from EDL"
          edl.each do |event|
            event[:areas].each do |area, videos|
        ...
        Averox.logger.info "Compositing cuts"
      ...
      def self.video_info(filename)
        IO.popen([*FFPROBE, filename]) do |probe|
          ...
          begin
            ...
            Averox.logger.warn("Couldn't parse video info: #{e}")
          end
          return {} if !info
      ...
      def self.composite_cut(output, cut, layout, videoinfo)
        ...
        Averox.logger.info "  Cut start time #{cut[:timestamp]}, duration #{duration}"
        ...
        layout[:areas].each do |layout_area|
          ...
          Averox.logger.debug "  Laying out #{video_count} videos in #{layout_area[:name]}"
          ...
          Averox.logger.debug "    Tiling in a #{tiles_h}x#{tiles_v} grid"

          ffmpeg_filter << "[#{layout_area[:name]}_in];"

          area.each do |video|
            this_videoinfo = videoinfo[video[:filename]]
            Averox.logger.debug "    tile location (#{tile_x}, #{tile_y})"
            video_width = this_videoinfo[:aspect_ratio].numerator
            video_height = this_videoinfo[:aspect_ratio].denominator
            Averox.logger.debug "      original aspect: #{video_width}x#{video_height}"

            scale_width, scale_height = aspect_scale(video_width, video_height, tile_width, tile_height)
            Averox.logger.debug "      scaled size: #{scale_width}x#{scale_height}"

            seek = video[:timestamp]
            Averox.logger.debug("      start timestamp: #{seek}")
            seek_offset = this_videoinfo[:start_time]
            video_start_offset = this_videoinfo[:video][:start_time]
            Averox.logger.debug("      seek offset: #{seek_offset}, video start offset: #{video_start_offset}")
            Averox.logger.debug("      codec: #{this_videoinfo[:video][:codec_name].inspect}")
            Averox.logger.debug("      duration: #{this_videoinfo[:duration]}, original duration: #{video[:original_duration]}")
            ...
            if !video[:original_duration].nil? and
              ...
              seek = 0
              Averox.logger.debug("      Early 1.1 deskshare timestamp bug: scaling video length by #{scale}")
            end
            ...
            begin
              ...
              Averox.logger.warn('Unable to increase pipe size to 1MB')
            rescue NameError
            ...
            Averox.logger.info("Executing: #{Shellwords.join(ffmpeg_preprocess_command)}")
            ...
            Averox.logger.debug("preprocessing ffmpeg command pid #{ffmpeg_preprocess_pid}")
        ...
        Averox.logger.debug('  ffmpeg filter_complex_script:')
        Averox.logger.debug(ffmpeg_filter)
        ...
        Averox.logger.info("Executing: #{Shellwords.join(ffmpeg_cmd)}")
        ...
        loop do
          ...
          if pid == ffmpeg_pid
            Averox.logger.debug("ffmpeg command #{exitstatus} (#{File.basename(ffmpeg_log)})")
            ...
            Averox.logger.debug("preprocessing ffmpeg command #{exitstatus} (#{File.basename(process[:log])})")
        ...
        Averox.logger.info('All ffmpeg processes exited normally')
