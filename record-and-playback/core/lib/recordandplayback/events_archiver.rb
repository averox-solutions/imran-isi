...
module Averox
  $avx_props = YAML::load(File.open(File.expand_path('../../../scripts/averox.yml', __FILE__)))
  $recording_dir = $avx_props['recording_dir']
  $raw_recording_dir = "#{$recording_dir}/raw"
  $store_recording_status = $avx_props['store_recording_status']
  ...
  class RedisWrapper
    ...
    def build_header(message_type)
      return {
        "timestamp" => Averox.monotonic_clock, #
        "name" => message_type,
    ...
    RECORDINGS_CHANNEL = "averox:from-rap"
    RAP_STATUS_LIST = "averox:rap:status"
  ...
  class RedisEventsArchiver
    ...
    def store_events(meeting_id, events_file, break_timestamp)
      version = Averox.read_props["avx_version"]
      ...
      msgs.each_with_index do |msg, i|
        ...
        if moved > 0
          Averox.logger.info("Reordered event timestamp=#{res[TIMESTAMP]} module=#{res[MODULE]} " \
