...
module Averox
  ...
  def Averox.process_webcam_videos(target_dir, raw_archive_dir, output_width, output_height, output_framerate, audio_offset, processed_audio_file, video_formats=['webm'], show_moderator_viewpoint=false)
    Averox.logger.info("Processing webcam videos")
    ...
    webcam_edl = Averox::Events.create_webcam_edl(
                    events, raw_archive_dir, show_moderator_viewpoint)
    user_video_edl = Averox::Events.edl_match_recording_marks_video(
                    webcam_edl, events, start_time, end_time)
    Averox::EDL::Video.dump(user_video_edl)
    ...
    user_video_file = Averox::EDL::Video.render(
    ...
    formats.each do |format|
      filename = Averox::EDL::encode(
        processed_audio_file, user_video_file, format, "#{target_dir}/webcams", audio_offset)
    end
  end

  def Averox.process_deskshare_videos(target_dir, raw_archive_dir, output_width, output_height, output_framerate, video_formats=['webm'])
    Averox.logger.info("Processing deskshare videos")
    ...
    start_time = Averox::Events.first_event_timestamp(events)
    end_time = Averox::Events.last_event_timestamp(events)
    deskshare_edl = Averox::Events.create_deskshare_edl(
                    events, raw_archive_dir)
    deskshare_video_edl = Averox::Events.edl_match_recording_marks_video(
                    deskshare_edl, events, start_time, end_time)

    return if not Averox.video_recorded?(deskshare_video_edl)

    Averox::EDL::Video.dump(deskshare_video_edl)
    ...
    deskshare_video_file = Averox::EDL::Video.render(
