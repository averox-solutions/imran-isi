...
module Averox
  module Events
    ...
    def self.get_meeting_metadata(events_xml)
      Averox.logger.info("Task: Getting meeting metadata")
    ...
    def self.get_notes_id(events)
      Averox.logger.info("Task: Getting notes id")
      notes_id = 'undefined'
      cc_token = '_cc_'
    ...
    def self.get_external_meeting_id(events_xml)
      Averox.logger.info("Task: Getting external meeting id")
    ...
    def self.find_video_event_matched(start_events, stop)
      Averox.logger.info("Task: Finding video events that match")
    ...
    def self.process_webcamsOnlyForModerator(list_user_info, active_videos, inactive_videos, webcamsOnlyForModerator)
      ...
      if webcamsOnlyForModerator
        list_user_info.each do |user_id, user_role|
          ...
          if !Averox::Events.is_user_moderator(user_id, list_user_info)
            filename = Averox::Events.extract_filename_from_userId(user_id, active_videos)
            if filename != ""
    ...
    def self.create_webcam_edl(events, archive_dir, show_moderator_viewpoint)
      ...
      if show_moderator_viewpoint
        events.xpath('/recording/event[@module="WEBCAM" or (@module="avx-webrtc-sfu" and (@eventname="StartWebRTCShareEvent" or @eventname="StopWebRTCShareEvent"))]').each do |event|
        ...
        events.xpath('/recording/event[@module="WEBCAM" or (@module="avx-webrtc-sfu" and (@eventname="StartWebRTCShareEvent" or @eventname="StopWebRTCShareEvent")) or (@module="PARTICIPANT" and (@eventname="ParticipantStatusChangeEvent" or @eventname="ParticipantJoinEvent")) or @eventname="WebcamsOnlyForModeratorEvent" or @eventname="MeetingConfigurationEvent"]').each do |event|
          timestamp = event['timestamp'].to_i - initial_timestamp
          ...
          case event['eventname']
          when 'StartWebcamShareEvent', 'StartWebRTCShareEvent'
            userId = Averox::Events.get_id_from_filename(filename)
            is_in_forbidden_period = webcamsOnlyForModerator

            if (!is_in_forbidden_period) || (is_in_forbidden_period && Averox::Events.is_user_moderator(userId, list_user_info))
              ...
              active_videos.each do |filename|
                edl_entry[:areas][:webcam] << {
                  :filename => filename,
                  :timestamp => timestamp - videos[filename][:timestamp],
                  :user_id => Averox::Events.get_id_from_filename(filename)
                }
              end
              video_edl << edl_entry
            elsif is_in_forbidden_period && !Averox::Events.is_user_moderator(userId, list_user_info)
              inactive_videos << filename
              videos[filename] = { :timestamp => timestamp }
            end
          when 'StopWebcamShareEvent', 'StopWebRTCShareEvent'
          ...
          when "ParticipantStatusChangeEvent"
            ...
            if event.at_xpath('status').text == "role" 
              ...
              if is_in_forbidden_period && event.at_xpath('value').text == "MODERATOR"
                filename_to_add = Averox::Events.extract_filename_from_userId(userId, inactive_videos)
                if filename_to_add != ""
                ...
                filename_to_add = Averox::Events.extract_filename_from_userId(userId, active_videos)
                if filename_to_add != ""
          ...
          when "MeetingConfigurationEvent"
            webcamsOnlyForModerator = Averox::Events.to_boolean(event.at_xpath('webcamsOnlyForModerator').text)

          when "WebcamsOnlyForModeratorEvent"
            webcamsOnlyElement = event.at_xpath("webcamsOnlyForModerator")
            # Handle typo in some Averox versions
            webcamsOnlyElement = event.at_xpath("webacmsOnlyForModerator") if webcamsOnlyElement.nil?

            webcamsOnlyForModerator = Averox::Events.to_boolean(webcamsOnlyElement.text)

            # Change active and inactive videos.
            Averox::Events.process_webcamsOnlyForModerator(list_user_info, active_videos, inactive_videos, webcamsOnlyForModerator)
    ...
    def self.get_matched_start_and_stop_deskshare_events(events)
      last_timestamp = Averox::Events.last_event_timestamp(events)
      deskshare_start_events = Averox::Events.get_start_deskshare_events(events)
      deskshare_stop_events = Averox::Events.get_stop_deskshare_events(events)
      return Averox::Events.match_start_and_stop_deskshare_events(
    ...
    def self.match_start_and_stop_deskshare_events(start_events, stop_events, last_timestamp)
      Averox.logger.info("Task: Matching the start and stop deskshare events")
      matched_events = []
      start_events.each do |start|
    ...
    def self.get_start_deskshare_events(events)
      start_events = []
      events.xpath('/recording/event[@module="Deskshare" or (@module="avx-webrtc-sfu" and @eventname="StartWebRTCDesktopShareEvent")]').each do |start_event|
    ...
    def self.get_stop_deskshare_events(events)
      Averox.logger.info("Task: Getting stop DESKSHARE events")
      stop_events = []
      events.xpath('/recording/event[@module="Deskshare" or (@module="avx-webrtc-sfu" and @eventname="StopWebRTCDesktopShareEvent")]').each do |stop_event|
    ...
    def self.create_deskshare_edl(events, archive_dir)
      initial_timestamp = Averox::Events.first_event_timestamp(events)
      final_timestamp = Averox::Events.last_event_timestamp(events)
      ...
      events.xpath('/recording/event[@module="Deskshare" or (@module="avx-webrtc-sfu" and (@eventname="StartWebRTCDesktopShareEvent" or @eventname="StopWebRTCDesktopShareEvent"))]').each do |event|
        timestamp = event['timestamp'].to_i - initial_timestamp
        # Determine the video filename
        case event['eventname']
    ...
    def self.edl_match_recording_marks_audio(edl, events, start_time, end_time)
      edl_entry_offset = Averox::Events.edl_entry_offset_audio
      edl_empty_entry = Averox::Events.edl_empty_entry_audio
      return Averox::Events.edl_match_recording_marks(edl, events,
    ...
    def self.edl_match_recording_marks_video(edl, events, start_time, end_time)
      edl_entry_offset = Averox::Events.edl_entry_offset_video
      edl_empty_entry = Averox::Events.edl_empty_entry_video
      return Averox::Events.edl_match_recording_marks(edl, events,
                      edl_entry_offset, edl_empty_entry, start_time, end_time)
    end
    ...
    def self.edl_apply_start_stop_events(edl, edl_entry_offset, edl_empty_entry, start_stop_events)
      ...
      start_stop_events.each do |start_stop_event|
        Averox.logger.debug("Recording interval: #{start_stop_event[:start_timestamp]} to #{start_stop_event[:stop_timestamp]}")
        offset += start_stop_event[:start_timestamp] - last_stop_timestamp
        Averox.logger.debug("Offset is now #{offset}")
        ...
        Averox.logger.debug("Found last event prior to recording start:")
        Averox.logger.debug(Averox.hash_to_str(edl[input_i]))

        # Add the first event, trimming the start as needed.
        initial_trim = start_stop_event[:start_timestamp] - edl[input_i][:timestamp]
        Averox.logger.debug("Have to trim #{initial_trim}")
        ...
        Averox.logger.debug("New event at recording start:")
        Averox.logger.debug(Averox.hash_to_str(new_edl[output_i]))
        output_i += 1
    ...
    def self.get_start_stop_events_for_edl(events, start_time, end_time)
      initial_timestamp = Averox::Events.first_event_timestamp(events)
      start_stop_events = Averox::Events.match_start_and_stop_rec_events(
              Averox::Events.get_start_and_stop_rec_events(events))
      start_stop_events = Averox::Events.trim_start_and_stop_rec_events(
    ...
    def self.edl_match_recording_marks(edl, events,
                                       ...
                                       start_time, end_time)
      start_stop_events = Averox::Events.get_start_stop_events_for_edl(events, start_time, end_time)
      return Averox::Events.edl_apply_start_stop_events(edl, edl_entry_offset, edl_empty_entry, start_stop_events)
    end
    ...
    # Reads the keys 'anonymize_chat' and 'anonymize_chat_moderators' from avx_props, but allows
    # per-meeting override using the create meta params 'meta_avx-anonymize-chat' and
    # 'meta_avx-anonymize-chat-moderators'
    ...
    def self.get_chat_events(events, start_time, end_time, avx_props = {})
      Averox.logger.info('Getting chat events')
      ...
      # Load the anonymize settings; defaults from averox.yml, override with meta params
      metadata = events.at_xpath('/recording/metadata')
      anonymize_senders = metadata['avx-anonymize-chat'] unless metadata.nil?
      anonymize_senders = avx_props['anonymize_chat'] if anonymize_senders.nil?
      anonymize_senders = anonymize_senders.to_s.casecmp?('true')
      anonymize_moderators = metadata['avx-anonymize-chat-moderators'] unless metadata.nil?
      anonymize_moderators = avx_props['anonymize_chat_moderators'] if anonymize_moderators.nil?
    ...
    def self.get_record_status_events(events_xml)
      Averox.logger.info "Getting record status events"
      rec_events = []
    ...
    def self.get_external_video_events(events_xml)
      Averox.logger.info "Getting external video events"
    ...
    def self.get_start_and_stop_rec_events(events_xml, allow_empty_events=false)
      Averox.logger.info "Getting start and stop rec button events"
      rec_events = Averox::Events.get_record_status_events(events_xml)
      if !allow_empty_events and rec_events.empty?
        # old recording generated in a version without the record button
        rec_events << { :timestamp => Averox::Events.first_event_timestamp(events_xml) }
      end
      if rec_events.size.odd?
        # user did not click on the record button to stop the recording
        rec_events << { :timestamp => Averox::Events.last_event_timestamp(events_xml) }
      end
      rec_events.sort_by {|a| a[:timestamp]}
    end
    ...
    def self.get_start_and_stop_external_video_events(events_xml)
      Averox.logger.info "Getting start and stop externalvideo events"
      external_video_events = Averox::Events.get_external_video_events(events_xml)
      if external_video_events.size.odd?
        # user did not click to stop external video before ending meeting
        external_video_events << { :timestamp => Averox::Events.last_event_timestamp(events_xml) }
      end
    ...
    def self.match_start_and_stop_rec_events(rec_events)
      Averox.logger.info ("Matching record events")
      matched_rec_events = []
    ...
    def self.match_start_and_stop_external_video_events(external_video_events)
      Averox.logger.info ("Matching external video events")
    ...
    def self.have_webcam_events(events_xml)
      Averox.logger.debug("Checking if webcams were used...")
      ...
      if webcam_events.length > 0
        Averox.logger.debug("Webcam events seen in recording")
        return true
      else
        Averox.logger.debug("No webcam events were seen in recording")
        return false
      end
    end
    ...
    def self.have_deskshare_events(events_xml)
      Averox.logger.debug('Checking if desktop sharing was used...')
      deskshare_events = events_xml.xpath('/recording/event[@module="Deskshare" or (@module="avx-webrtc-sfu" and @eventname="StartWebRTCDesktopShareEvent")]')
      if deskshare_events.length > 0
        Averox.logger.debug('Deskshare events seen in recording')
        true
      else
        Averox.logger.debug('No deskshare events were seen in recording')
        false
      end
    end
    ...
    def self.have_presentation_events(events_xml)
      Averox.logger.debug("Checking if presentation area was used...")
      ...
      pres_events.each do |event|
        ...
        Averox.logger.debug("Seen a #{event['eventname']} event, presentation area used.")
          return true
        # We ignore the first SharePresentationEvent, since it's the default
        # presentation
        when 'SharePresentationEvent'
          if seen_share_presentation
            Averox.logger.debug("Have a non-default SharePresentation")
            return true
          else
            Averox.logger.debug("Skipping default SharePresentation")
        ...
        when 'GotoSlideEvent'
          ...
          if slide != 0
            Averox.logger.debug("Switched to slide #{slide}")
            return true
          end
          Averox.logger.debug("Ignoring GotoSlide with default slide #")
        end
      end
      Averox.logger.debug("No important presentation events found")
      return false
    end
    ...
    def self.get_segment_start_timestamp(events_xml, break_timestamp)
      ...
      if segment_i > 0
        ...
        return Averox::Events.first_event_timestamp(events_xml)
      end
    end
    ...
    def self.get_segment_end_timestamp(events_xml, break_timestamp)
      if !break_timestamp.nil?
        ...
        return Averox::Events.last_event_timestamp(events_xml)
      end
    end

    # Version of the avx server where it was recorded
    def self.avx_version(events)
      recording = events.at_xpath('/recording')
      recording['avx_version']
    end

    # Compare version numbers
    # Returns true if version is newer than requested version
    def self.avx_version_compare(events, major, minor=nil, micro=nil)
      avx_version = self.avx_version(events)
      if avx_version.nil?
        # Averox 0.81 or earler
        return false
      end

      # Split the version string
      match = /^(\d+)\.(\d+)\.(\d+)/.match(avx_version)
      if !match
        raise "avx_version #{avx_version} is not in the correct format"
      end
    ...
    def self.screenshare_has_audio?(events, deskshare_dir)
      events.xpath('/recording/event[@eventname="StartWebRTCDesktopShareEvent"]').each do |event|
        ...
        fileHasAudio = !Averox::EDL::Audio.audio_info(filename)[:audio].nil?
        if fileHasAudio
          return true
        end
      end
