...
module Averox
  class AudioEvents
    ...
    def self.create_deskshare_audio_edl(events, deskshare_dir)
      ...
      events.xpath('/recording/event[@module="avx-webrtc-sfu" and (@eventname="StartWebRTCDesktopShareEvent" or @eventname="StopWebRTCDesktopShareEvent")]').each do |event|
        filename = event.at_xpath('filename').text
        ...
        if (fileHasAudio)
          ...
          Averox.logger.debug " Screenshare without audio, ignoring..."
        end
      end
