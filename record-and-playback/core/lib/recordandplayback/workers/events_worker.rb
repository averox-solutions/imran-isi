  ...
  module Resque
    class EventsWorker < BaseWorker
      ...
      def store_events(target_dir)
        ...
        redis = Averox::RedisWrapper.new(@props['redis_host'], @props['redis_port'], @props['redis_password'])
      ...
      def store_etherpad_events(target_dir)
        ...
        Averox.try_download("#{@notes_endpoint}/#{CGI.escape notes_id}/export/etherpad", events_etherpad)
      end
      ...
      def initialize(opts)
        ...
        @post_scripts_path = File.join(Averox.rap_scripts_path, 'post_events')
