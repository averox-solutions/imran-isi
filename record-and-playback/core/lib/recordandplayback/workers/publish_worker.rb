    ...
    class PublishWorker < BaseWorker
      ...
      def perform
        super do
          ...
          script = File.join(Averox.rap_scripts_path,
          ...
          if File.exist?(script)
            ...
            if File.exist?(metadata_xml_path)
              begin
                ...
                Averox.logger.warn 'An exception occurred while loading the extra information for the publish event'
                Averox.logger.warn e.message
                e.backtrace.each do |traceline|
                  Averox.logger.warn traceline
                end
              end
            else
              Averox.logger.warn "Couldn't find the metadata file at #{metadata_xml_path}"
            end
      ...
      def initialize(opts)
        ...
        @post_scripts_path = File.join(Averox.rap_scripts_path, 'post_publish')
