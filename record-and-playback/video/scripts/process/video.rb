...
# This file is part of Averox.
#
# Copyright © Averox Inc. and by respective authors.
#
...
# along with Averox. If not, see <https://www.gnu.org/licenses>.
...
props = YAML.safe_load(File.open(File.expand_path('../averox.yml', __dir__)))
...
duration = Averox::Events.get_recording_length(events)
participants = Averox::Events.get_num_participants(events)
metadata = events.at_xpath('/recording/metadata')

logger.info 'Generating video events list'

# Webcams
webcam_edl = Averox::Events.create_webcam_edl(events, raw_archive_dir, props['show_moderator_viewpoint'])
logger.debug 'Webcam EDL:'
Averox::EDL::Video.dump(webcam_edl)

# Deskshare
deskshare_edl = Averox::Events.create_deskshare_edl(events, raw_archive_dir)
logger.debug 'Deskshare EDL:'
Averox::EDL::Video.dump(deskshare_edl)

video_edl = Averox::EDL::Video.merge(webcam_edl, deskshare_edl)

logger.debug 'Merged Video EDL:'
Averox::EDL::Video.dump(video_edl)

logger.info 'Applying recording start/stop events to video'
video_edl = Averox::Events.edl_match_recording_marks_video(video_edl, events, initial_timestamp, final_timestamp)
logger.debug 'Trimmed Video EDL:'
Averox::EDL::Video.dump(video_edl)

logger.info 'Checking whether webcams were used'
have_webcams = Averox::Events.have_webcam_events(events)
if have_webcams
...
have_deskshare = Averox::Events.have_deskshare_events(events)
if have_deskshare
...
have_presentation = Averox::Events.have_presentation_events(events)
...
Averox::EDL::Video.dump(presentation_edl)

video_edl = Averox::EDL::Video.merge(presentation_edl, video_edl)
logger.debug 'Merged Video EDL with Presentation:'
Averox::EDL::Video.dump(video_edl)

logger.info 'Generating audio events list'
audio_edl = Averox::AudioEvents.create_audio_edl(events, raw_archive_dir)
logger.debug 'Audio EDL:'
Averox::EDL::Audio.dump(audio_edl)

logger.info 'Applying recording start/stop events to audio'
audio_edl = Averox::Events.edl_match_recording_marks_audio(audio_edl, events, initial_timestamp, final_timestamp)
logger.debug 'Trimmed Audio EDL:'
Averox::EDL::Audio.dump(audio_edl)

logger.info 'Rendering audio'
audio = Averox::EDL::Audio.render(audio_edl, "#{process_dir}/audio")

if Averox::Events.screenshare_has_audio?(events, "#{raw_archive_dir}/deskshare")
  logger.info('Generating audio events list for deskshare')
  deskshare_audio_edl = Averox::AudioEvents.create_deskshare_audio_edl(events, "#{raw_archive_dir}/deskshare")
  ...
  deskshare_audio = Averox::EDL::Audio.render(deskshare_audio_edl, "#{process_dir}/deskshare_audio")

  logger.info('Mixing meeting audio and deskshare audio')
  audio = Averox::EDL::Audio.mixer([audio, deskshare_audio], "#{process_dir}/mixed_audio")
end
...
video = Averox::EDL::Video.render(video_edl, layout, "#{process_dir}/video")
...
video_props['formats'].each_with_index do |format, i|
  ...
  Averox::EDL.encode(audio, video, format, "#{process_dir}/video-#{i}", video_props['audio_offset'])
end

logger.info('Generating closed captions')
ret = Averox.exec_ret('utils/gen_webvtt', '-i', raw_archive_dir, '-o', process_dir)
...
begin
  ...
  chats = Averox::Events.get_chat_events(events, initial_timestamp, final_timestamp, props)
