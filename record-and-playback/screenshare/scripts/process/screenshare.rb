...
#
# Copyright (c) 2018 Averox Inc. and by respective authors.
#
...
props = YAML::load(File.open(File.expand_path('../../averox.yml', __FILE__)))
...
if opts[:stderr]
  Averox.logger = Logger.new(STDERR)
else
  Averox.logger = Logger.new(log_file)
end
logger = Averox.logger
...
video_edl = Averox::Events.edl_match_recording_marks_video(video_edl, events, start_time, end_time)
...
audio_edl = Averox::Events.edl_match_recording_marks_audio(audio_edl, events, start_time, end_time)
logger.debug "Trimmed Audio EDL:"
Averox::EDL::Audio.dump(audio_edl)

logger.info "Rendering audio"
audio = "#{process_dir}/audio.#{Averox::EDL::Audio::WF_EXT}"
if File.exist?(audio)
...
video = "#{process_dir}/video.#{Averox::EDL::Video::WF_EXT}"
if File.exist?(video)
...
screenshare_props['formats'].each_with_index do |format, i|
  ...
  if File.exist?(filename)
    ...
    filename = Averox::EDL.encode(audio, video, format, "#{process_dir}/screenshare-#{i}", 0)
  end
end
