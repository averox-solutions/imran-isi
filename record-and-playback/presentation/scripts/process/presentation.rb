...
unless FileTest.directory?(target_dir)
  ...
  Averox.logger.info('Processing script presentation.rb')
  ...
  begin
    ...
    metadata_xml.close
    Averox.logger.info('Created inital metadata.xml')

    Averox::AudioProcessor.process(raw_archive_dir, "#{target_dir}/audio")
    ...
    presentations = Averox::Presentation.get_presentations(events_xml)
    ...
    participants.content = Averox::Events.get_num_participants(@doc)
    ...
    Nokogiri::XML::Builder.with(metadata.at('recording')) do |xml|
      xml.meta do
        Averox::Events.get_meeting_metadata("#{target_dir}/events.xml").each { |k, v| xml.method_missing(k, v) }
      end
    end
    ...
    metadata_file.close
    Averox.logger.info('Created an updated metadata.xml with start_time and end_time')

    version_atleast_2_6_0 = Averox::Events.avx_version_compare(@doc, 2, 6, 0)
    # Start processing raw files
    presentation_text = {}
    presentations.each do |pres|
      pres_dir = "#{presentation_dir}/#{pres}"
      num_pages = Averox::Presentation.get_number_of_pages_for(pres_dir)
      ...
      if images.empty?
        ...
        if File.exist?("#{pres_name}.pdf")
          ...
          Averox.logger.info("Found pdf file for presentation #{pres_pdf}")
        elsif File.exist?("#{pres_name}.PDF")
          pres_pdf = "#{pres_name}.PDF"
          Averox.logger.info("Found PDF file for presentation #{pres_pdf}")
        elsif File.exist?(pres_name.to_s)
          pres_pdf = pres_name
          Averox.logger.info("Falling back to old presentation filename #{pres_pdf}")
        else
          pres_pdf = ''
          Averox.logger.warn("Could not find pdf file for presentation #{pres}")
        end

        unless pres_pdf.empty?
          text = {}
          1.upto(num_pages) do |page|
            Averox::Presentation.extract_png_page_from_pdf(
        ...
        Averox::Presentation.convert_image_to_png(
    ...
    if !Dir["#{raw_archive_dir}/video/*"].empty? ||
      ...
      processed_audio_file = Averox::AudioProcessor.get_processed_audio_file(raw_archive_dir, "#{target_dir}/audio")
      Averox.process_webcam_videos(target_dir, raw_archive_dir, webcam_width, webcam_height, webcam_framerate, presentation_props['audio_offset'], processed_audio_file, presentation_props['video_formats'], props['show_moderator_viewpoint'])
    end
    ...
    if !Dir["#{raw_archive_dir}/deskshare/*"].empty? && presentation_props['include_deskshare']
      ...
      Averox.process_deskshare_videos(target_dir, raw_archive_dir, deskshare_width, deskshare_height, deskshare_framerate, presentation_props['video_formats'])
    end
    ...
    metadata_file.write(metadata.root)
    metadata_file.close
    Averox.logger.info('Created an updated metadata.xml with state=processed')
