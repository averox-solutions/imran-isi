...
avx_props = Averox.read_props
@presentation_props = YAML.safe_load(File.read('presentation.yml'))
filepathPresOverride = "/etc/averox/recording/presentation.yml"
...
def get_deskshare_video_dimension(deskshare_stream_name)
  ...
  if File.exist?(deskshare_video_filename)
    video_info = Averox::EDL::Video.video_info(deskshare_video_filename)
    video_width, video_height = scale_to_deskshare_video(video_info[:width], video_info[:height])
  else
    Averox.logger.error("Could not find deskshare video: #{deskshare_video_filename}")
  end
...
def svg_render_shape_pencil(g, slide, shape)
  ...
  if data_points_length < 2
    Averox.logger.warn("Pencil #{shape_unique_id} doesn't have enough points")
    return
  end
...
def svg_render_shape_rect(g, slide, shape)
  ...
  if shape[:square]
    ...
    if x2 > x1
      ...
      # This replicates a bug in the Averox flash client
      Averox.logger.info("Rect #{shape[:shape_unique_id]} reversed square bug")
      y2 = y1 - width
    end
  end
...
def svg_render_shape_ellipse(g, slide, shape)
  ...
  if shape[:circle]
    ...
    if x2 > x1
      ...
      # This replicates a bug in the Averox flash client
      Averox.logger.info("Ellipse #{shape[:shape_unique_id]} reversed circle bug")
      y2 = y1 - height_r - height_r
    end
  end
...
def svg_render_shape_poll(g, slide, shape)
  result = shape[:result]
  if result == "[]"
    Averox.logger.info("Poll #{shape[:shape_unique_id]} result is empty (no options/answers), ignoring...")
    return
  end
  ...
  # Render the poll svg
  ret = Averox.exec_ret('utils/gen_poll_svg', '-i', json_file, '-w', width.round.to_s, '-h', height.round.to_s,
...
def build_tldraw_shape(image_shapes, slide, shape)
  ...
  if shape_in == shape_out
    Averox.logger.info("Draw #{shape[:shape_id]} Shape #{shape[:shape_unique_id]} is never shown (duration rounds to 0)")
    return
  end

  if (shape_in >= slide[:out]) || (!shape[:out].nil? && shape[:out] <= slide[:in])
    Averox.logger.info("Draw #{shape[:shape_id]} Shape #{shape[:shape_unique_id]} is not visible during image time span")
    return
  end
...
def svg_render_shape(canvas, slide, shape, image_id)
  ...
  when 'poll_result'
    ...
    Averox.logger.warn("Ignoring unhandled shape type #{shape[:type]}")
  end
...
def svg_render_image(svg, slide, shapes, tldraw, tldraw_shapes)
  ...
  if slide_in == slide_out || slide_in > (@recording_time / 1000)
    Averox.logger.info("Presentation #{presentation} Slide #{slide_number} is never shown (duration rounds to 0)")
    return
  end
  ...
  Averox.logger.info("Image #{image_id}: Presentation #{presentation} Slide #{slide_number} Deskshare #{slide_deskshare} from #{slide_in} to #{slide_out}")
...
def events_parse_shape(shapes, event, current_presentation, current_slide, timestamp)
  ...
  if %w[ellipse line pencil rectangle triangle].include?(shape_type)
    ...
    unless thickness
      Averox.logger.warn("Draw #{draw_id} Shape #{shape[:shape_unique_id]} ID #{shape_id} is missing thickness")
      return
    end
  ...
  when 'text'
    ...
    unless calced_font_size
      Averox.logger.warn("Draw #{draw_id} Shape #{shape[:shape_unique_id]} ID #{shape_id} is missing calcedFontSize")
      return
    end
  ...
  if prev_shape
    ...
    if (shape_type == 'pencil') && (shape_status == 'DRAW_UPDATE')
      # Averox 2.0 quirk - 'DRAW_UPDATE' events on pencil tool only
...
def events_parse_undo(shapes, event, current_presentation, current_slide, timestamp)
  ...
  if shape_id
    ...
    Averox.logger.info("Undo: removing shape with ID #{shape_id} at #{timestamp}")
    ...
    if undo_shape
      ...
      Averox.logger.info("Undo: removing Shape #{undo_shape_unique_id} at #{timestamp}")
      ...
      Averox.logger.info('Undo: no applicable shapes found')
    end
  end
end
...
def events_parse_clear(shapes, event, current_presentation, current_slide, timestamp)
  ...
  # Averox 2.0 per-user clear features; default to full clear on older versions
  ...
  if full_clear
    Averox.logger.info("Clear: removing all shapes")
  else
    Averox.logger.info("Clear: removing shapes for User #{user_id}")
  end
...
def events_get_image_info(slide, tldraw)
  ...
  unless File.exist?(image_path)
    Averox.logger.warn("Missing image file #{image_path}!")
    ...
    Averox.exec_ret(*command) || raise("Unable to generate blank image for #{image_path}")
  end

  slide[:width], slide[:height] = FastImage.size(image_path)
  Averox.logger.info("Image size is #{slide[:width]}x#{slide[:height]}")
end
...
def process_presentation(package_dir)
  ...
  tldraw_shapes = {'avx_version': Averox::Events.avx_version(@doc)}
  ...
  events_xml.xpath('/recording/event').each do |event|
    ...
    if slide_changed
      ...
      if slide &&
        ...
        Averox.logger.info('Presentation/Slide: skipping, no changes')
      else
        if slide
          slide[:out] = timestamp
        ...
        Averox.logger.info("Presentation #{current_presentation} Slide #{current_slide} Deskshare #{deskshare}")
        slide = {
    ...
    if panzoom_changed
      ...
      if panzoom &&
        ...
        Averox.logger.info('Panzoom: skipping, no changes')
        ...
        Averox.logger.info("Panzoom: #{current_x_offset} #{current_y_offset} #{current_width_ratio} #{current_height_ratio} (#{slide_width}x#{slide_height})")
        panzoom = {
    ...
    if cursor_changed || panzoom_changed
      ...
      if cursor &&
          ...
          !panzoom_changed
        Averox.logger.info('Cursor: skipping, no changes')
      else
        if cursor
...
def process_chat_messages(events, avx_props)
  Averox.logger.info('Processing chat events')
  # Create slides.xml and chat.
  xml = Builder::XmlMarkup.new(indent: 2)
  xml.instruct!
  xml.popcorn do
    Averox::Events.get_chat_events(events, @meeting_start.to_i, @meeting_end.to_i, avx_props).each do |chat|
      chattimeline = {
...
def process_deskshare_events(events)
  Averox.logger.info('Processing deskshare events')
  deskshare_matched_events = Averox::Events.get_matched_start_and_stop_deskshare_events(events)
  ...
  @deskshare_xml.recording('id' => 'deskshare_events') do
    deskshare_matched_events.each do |event|
      ...
      video_info = Averox::EDL::Video.video_info("#{@deskshare_dir}/#{event_stream = event[:stream]}")
      unless video_info[:video]
        Averox.logger.warn("#{event_stream} is not a valid video file, skipping...")
        next
      end
...
def process_poll_events(events, package_dir)
  Averox.logger.info('Processing poll events')
...
def process_external_video_events(_events, package_dir)
  Averox.logger.info('Processing external video events')

  # Retrieve external video events
  external_video_events = Averox::Events.match_start_and_stop_external_video_events(
    Averox::Events.get_start_and_stop_external_video_events(@doc)
  )

  external_videos = []
  @rec_events.each do |re|
    external_video_events.each do |event|
      Averox.logger.info("Processing rec event #{re} and external video event #{event}")
      start_timestamp = event[:start_timestamp]
...
def copy_media_files_helper(media, media_files, package_dir)
  Averox.logger.info("Making #{media} dir")
  FileUtils.mkdir_p(media_dir = "#{package_dir}/#{media}")

  media_files.each do |media_file|
    Averox.logger.info("Made #{media} dir - copying: #{media_file} to -> #{media_dir}")
    FileUtils.cp(media_file, media_dir)
    Averox.logger.info("Copied #{File.extname(media_file)} file")
  end
end
...
begin
  if @playback == 'presentation'
    log_dir = avx_props['log_dir']
    logger = Logger.new("#{log_dir}/presentation/publish-#{@meeting_id}.log", 'daily')
    Averox.logger = logger

    Averox.logger.info('Setting recording dir')
    @recording_dir = avx_props['recording_dir']

    Averox.logger.info('Setting process dir')
    @process_dir = "#{@recording_dir}/process/presentation/#{@meeting_id}"

    Averox.logger.info('Setting publish dir')
    publish_dir = @presentation_props['publish_dir']

    Averox.logger.info('Setting playback url info')
    playback_protocol = avx_props['playback_protocol']
    playback_host = avx_props['playback_host']

    Averox.logger.info('Setting target dir')
    ...
    if FileTest.directory?(target_dir)
      Averox.logger.info("#{target_dir} is already there")
    else
      Averox.logger.info('Making dir target_dir')
      FileUtils.mkdir_p target_dir

      package_dir = "#{target_dir}/#{@meeting_id}"
      Averox.logger.info('Making dir package_dir')
      ...
      begin
        ...
        @rec_events = Averox::Events.match_start_and_stop_rec_events(
          Averox::Events.get_start_and_stop_rec_events(@doc)
        )

        @recording_time = Averox::Events.get_recording_length(@doc)
        @meeting_start = Averox::Events.first_event_timestamp(@doc)
        @meeting_end = Averox::Events.last_event_timestamp(@doc)

        @version_atleast_0_9_0 = Averox::Events.avx_version_compare(
          @doc, 0, 9, 0
        )
        @version_atleast_2_0_0 = Averox::Events.avx_version_compare(
          @doc, 2, 0, 0
        )
        @version_atleast_2_6_0 = Averox::Events.avx_version_compare(
          @doc, 2, 6, 0
        )
        Averox.logger.info('Creating metadata.xml')
        ...
        Averox.logger.info('Copied metadata.xml file')
        ...
        presentation = Averox::Presentation.get_presentation_for_preview(@process_dir.to_s)
        ...
        Averox.logger.info('Added playback to metadata.xml')

        # Create slides.xml
        Averox.logger.info('Generating xml for slides and chat')

        calculate_record_events_offset

        # Write slides.xml to file
        slides_doc = process_chat_messages(@doc, avx_props)
        ...
        Averox.logger.info('Copying files to package dir')
        FileUtils.cp_r("#{@process_dir}/presentation", package_dir)
        Averox.logger.info('Copied files to package dir')

        Averox.logger.info('Publishing slides')
        ...
        Averox.add_raw_size_to_metadata(package_dir, raw_dir)
        Averox.add_playback_size_to_metadata(package_dir)

        FileUtils.cp_r(package_dir, publish_dir) # Copy all the files.
        Averox.logger.info('Finished publishing script presentation.rb successfully.')

        Averox.logger.info('Removing processed and published files.')
