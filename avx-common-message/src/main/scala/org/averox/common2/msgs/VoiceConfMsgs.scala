...
/**
 * Sync screenshare state with avx-html5
 */
...
/* Sent by avx-webrtc-sfu to ask permission for broadcasting a screen stream
 */
...
/* Sent to avx-webrtc-sfu to grant or deny screen sharing permission
 */
...
/* Sent by avx-webrtc-sfu to ask permission for subscring to a broadcasted
 * screen stream
 */
...
/* Sent to avx-webrtc-sfu to grant or deny a screen sharing subscribe request
 */
...
/**
 * Sent to avx-webrtc-sfu to tear down screen stream #streamId
 */
...
/**
 * Sent from avx-voice when a user connects to "global audio"
 */
...
/**
 * Sent from avx-voice when a user disconnects from "global audio"
 */
...
/* Sent by avx-webrtc-sfu to ask permission for adding a listener to the global
 * audio bridge
 */
...
/* Sent by avx-webrtc-sfu to ask permission for a new microphone/full audio
 * connection
 *   - callerIdNum: the session's callerId as assembled by the requester
 *   - sfuSessionId: the UUID for this request's session in avx-webrtc-sfu.
...
/* Sent to avx-webrtc-sfu as a response to GetMicrophonePermissionReqMsg
 *   - sfuSessionId: the UUID for this request's session in avx-webrtc-sfu.
 *     Used for response matching.
...
/**
 * Sent to avx-webrtc-sfu to request for userId's microphone connection
...
/**
 * Sent from avx-webrtc-sfu to indicate that userId's microphone channel switched
