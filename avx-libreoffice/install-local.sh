...
IMAGE_CHECK=`docker image inspect avx-soffice &> /dev/null && echo 1 || echo 0`
...
if [ "$FOLDER_CHECK" = "0" ]; then
	...
	install -Dm755 assets/convert-local.sh /usr/share/avx-libreoffice-conversion/convert.sh
	install -Dm755 assets/convert-cool.sh /usr/share/avx-libreoffice-conversion/convert-cool.sh
	install -Dm755 assets/etherpad-export.sh /usr/share/avx-libreoffice-conversion/etherpad-export.sh
	chown -R root /usr/share/avx-libreoffice-conversion/
else
...
if [ "$FILE_SUDOERS_CHECK" = "0" ]; then
	...
	cp assets/zzz-avx-docker-libreoffice /etc/sudoers.d/zzz-avx-docker-libreoffice
	chmod 0440 /etc/sudoers.d/zzz-avx-docker-libreoffice
	chown root:root /etc/sudoers.d/zzz-avx-docker-libreoffice
else
