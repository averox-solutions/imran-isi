...
# Conversion of office files to Pdf using local docker avx-soffice
...
mkdir -p "/tmp/avx-soffice-$(whoami)/"
tempDir="$(mktemp -d -p /tmp/avx-soffice-$(whoami)/)"
...
sudo /usr/bin/docker run --rm --memory=1g --memory-swap=1g --network none --env="HOME=/tmp/" -w /tmp/ --user=$(printf %05d `id -u`) -v "$tempDir/":/data/ -v /usr/share/fonts/:/usr/share/fonts/:ro --rm avx-soffice sh -c "timeout $(printf %03d $timeoutSecs)s /usr/bin/soffice -env:UserInstallation=file:///tmp/ $convertToParam --outdir /data /data/file"
