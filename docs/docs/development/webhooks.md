...
This is a node.js application that listens for all events on Averox and sends POST requests
...
Averox session. An event can be: a meeting was created, a user joined, a new presentation was
uploaded, a user left, a recording is being processed, and many others. You can see the entire list
of events that generate web hooks calls in [this file](https://github.com/averox/avx-webhooks/blob/fea98d5969050cec792b2a5890f58fd91ce57872/src/process/event.js#L8-L54).

## Installation

To install webhooks, run the following command on your Averox server:
...
This application adds three new API calls to Averox's API.
...
**Resource URL:** `http://yourserver.com/averox/api/hooks/create?[parameters]&checksum=[checksum]`
...
**Resource URL:** `http://yourserver.com/averox/api/hooks/destroy?[parameters]&checksum=[checksum]`
...
**Resource URL:** `http://yourserver.com/averox/api/hooks/list?[parameters]&checksum=[checksum]`
...
```javascript
event={"envelope":{"name":"MeetingDestroyedEvtMsg","routing":{"sender":"avx-apps-akka"}},"core":{"header":{"name":"MeetingDestroyedEvtMsg"},"body":{"meetingId":"44ea85d9684005d3b0af3c49e8a271a683cedb79-1532718208098"}}}
timestamp=1532718316953
```
...
* `<shared secret>`: The shared secret of the Averox server.
...
* Concatenate the original callback URL, the string from the previous step, and the Averox's salt.
...
* If a meeting is created while the web hooks application is down, callbacks will never be triggered
  ...
  is always running while Averox is running!
...
The easiest way to test the web hooks application is to register hooks in your Averox server
...
There's also a test server available at the avx-webhooks source code that can be used for catching hooks,
see: https://github.com/averox/avx-webhooks/blob/develop/extra/interceptor.js.
...
See [Webhooks docs in GitHub](https://github.com/averox/avx-webhooks).
