...
title: API Reference
sidebar_position: 4
description: Averox API Reference
keywords:
- api
---
...
This document describes the Averox application programming interface (API).
...
To make an API call to your Averox server, your application makes HTTPS requests to the Averox server API endpoint (usually the server's hostname followed by `/averox/api`). All API calls must include checksum computed with a secret shared with the Averox server.

The Averox server returns an XML response to all API calls.

### Updates to API in Averox

Updated in 0.9.0:

- **join** - `configToken` can now reference a file in `/var/averox/configs`, such as `myconfig.xml`.
...
- **join**: Added `userdata-avx_hide_presentation_on_join`.
...
- **join** - **Added:** `redirectErrorUrl`, `userdata-avx_fullaudio_bridge`
...
- **join** - **Added:** `enforceLayout`, `userdata-avx_default_layout`. **Removed:** `defaultLayout` (replaced by `userdata-avx_default_layout`) and removed support for all HTTP request methods except GET.
...
**String:**<br /> This data type indicates a (UTF-8) encoded string. When passing String values to Averox API calls, make sure that you use correctly URL-encoded UTF-8 values so international text will show up correctly. The string must not contain control characters (values 0x00 through 0x1F).

Some Averox API parameters put additional restrictions on which characters are allowed, or on the lengths of the string. These restrictions are described in the parameter documentation.
...
The Averox API security model enables 3rd-party applications to make API calls (if they have the shared secret), but not allow other people (end users) to make API calls.

The Averox API calls are almost all made server-to-server. If you installed the package `avx-demo` on your Averox server, you get a set of API examples, written in Java Server Pages (JSP), that demonstrate how to use the Averox API. These demos run as a web application in tomcat7. The web application makes HTTPS requests to the Averox server's API end point.

You can retrieve your Averox API parameters (API endpoint and shared secret) using the command
...
```
    URL: http://avx.example.com/averox/
    Secret: ECCJZNJWLPEA3YB6Y2LTQGQD3GJZ3F93
```

You should _not_ embed the shared secret within a web page and make Averox API calls within JavaScript running within a browser. The built-in debugging tools for modern browser would make this secret easily accessible to any user. Once someone has the shared secret for your Averox server, they could create their own API calls. The shared secret should only be accessible to the server-side components of your application (and thus not visible to end users).

### Configuration

The shared secret is located in the `/etc/averox/avx-web.properties` file.
...
We'll refer to this value as `sharedSecret`. When you first install Averox on a server, the packaging scripts create a random 32 character `sharedSecret`. You can also change the `sharedSecret` at anytime using the command `avx-conf --setsecret`.

```bash
$ sudo avx-conf --setsecret <new_shared_secret>
```

The following command will create a new 32 character shared secret for your server

```bash
$ sudo avx-conf --setsecret \$(openssl rand -base64 32 | sed 's/=//g' | sed 's/+//g' | sed 's/\///g')
```

**IMPORTANT: DO NOT ALLOW END USERS TO KNOW YOUR SHARED SECRET OR ELSE YOUR SECURITY WILL BE COMPROMISED.**

There are other configuration values in avx-web's configuration `averox.properties` (overwritten by `/etc/averox/avx-web.properties` ) related to the lifecycle of a meeting. You don't need to understand all of these to start using the Averox API. For most Averox servers, you can leave the [default values](https://github.com/averox/averox/blob/main/averox-web/grails-app/conf/averox.properties).

In Averox 2.5 support for additional hashing algorithms, besides sha1 and sha256, were added. These include sha384 and sha512. The `supportedChecksumAlgorithms` property in avx-web defines which algorithms are supported. By default checksums can be validated with any of the supported algorithms. To remove support for one or more of these algorithms simply delete it from the configuration file.
...
In `/etc/averox/avx-web.properties`:

```properties
supportedChecksumAlgorithms=sha512
```

In `/etc/averox/avx-apps-akka.conf`:
...
The Averox server will automatically remove empty meetings that were created but have never had any users after a number of minutes specified by `meetingExpireIfNoUserJoinedInMinutes` defined in avx-web's properties.

**Resource URL:**

http&#58;//yourserver.com/averox/api/create?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/create?name=Test&meetingID=test01&checksum=1234
- http&#58;//yourserver.com/averox/api/create?name=Test&meetingID=test01&moderatorPW=mp&attendeePW=ap&checksum=wxyz
- http&#58;//yourserver.com/averox/api/create?name=Test&meetingID=test01&moderatorPW=mp&attendeePW=ap&meta_presenter=joe&meta_category=education&checksum=abcd
...
<response>
  ...
  <parentMeetingID>avx-none</parentMeetingID>
...
#### POST request
You can also include a payload in the request, it may be useful in cases where some of the query parameters are big enough to exceed the maximum number of characters in URLs. Averox supports a POST request where the parameters that usually would be passed in the URL, can be sent through the body, see example below:

```bash
curl --request POST \
  --url https://<your-host>/averox/api/create \
...
You can upload slides within the create call. If you do this, the Averox server will immediately download and process the slides.
...
When you need to provide a document using a URL, and the document URL does not contain an extension, you can use the `filename` parameter, such as `filename=test-results.pdf` to help the Averox server determine the file type (in this example it would be a PDF file).
...
For more information about the pre-upload slides check the following [link](http://groups.google.com/group/averox-dev/browse_thread/thread/d36ba6ff53e4aa79).

#### clientSettingsOverride

You can modify the `settings.yml` configuration for the HTML5 client as part of the create call (in addition to modifying `/etc/averox/avx-html5.yml`).
...
<modules>
   <module name="clientSettingsOverride">
         <![CDATA[
         {
            "public": {
               "kurento": {
                  "wsUrl": "wss://test.averox.org//avx-webrtc-sfu"
               },
               "media": {
...
#### Upload slides from external application to a live Averox session

For external applications that integrate to Averox using the [insertDocument](/development/api#insertdocument) API call, `presentationUploadExternalUrl` and `presentationUploadExternalDescription` parameters can be used in the `create` API call in order to display a button and a message in the bottom of the presentation upload dialog. 
...
You can ask the Averox server to make a callback to your application when the meeting ends. Upon receiving the callback your application could, for example, change the interface for the user to hide the 'join' button.

To specify the callback to Averox, pass a URL using the meta-parameter `meta_endCallbackUrl` on the `create` command. When the Averox server ends the meeting, it will check if `meta_endCallbackUrl` is sent URL and, if so, make a HTTP GET request to the given URL.
...
Later, when the meeting ends, Averox will make an HTTPS GET request to this URL (HTTPS is supported and recommended) and to the URL add an additional parameter: `recordingmarks=true|false`.
...
You can ask the Averox server to make a callback to your application when the recording for a meeting is ready for viewing. Upon receiving the callback your application could, for example, send the presenter an e-mail to notify them that their recording is ready.

To specify the callback to Averox, pass a URL using the meta-parameter `meta_avx-recording-ready-url` on the `create` command. Later, when the Averox server finishes processing the recording, it will check if `meta_avx-recording-ready-url` is set and, if so, make a HTTP POST request to the given URL.
...
to pass this to Averox add the following parameter to the `create` API call: `&meta_avx-recording-ready-url=https%3A%2F%2Fexample.com%2Fapi%2Fv1%2Frecording_status` (note the callback URL needs to be URLEncoded).

Later, when the recording is ready, the Averox server will make an HTTPS POST request to this URL (https is supported and recommended).
...
- `meeting_id` - The value will be the meeting_id (as provided on the Averox create API call).
...
The secret used to sign the JWT message will be the shared secret of the Averox API endpoint that was used to create the original meeting.
...
http&#58;//yourserver.com/averox/api/join?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/join?meetingID=test01&password=mp&fullName=John&checksum=1234
- http&#58;//yourserver.com/averox/api/join?meetingID=test01&password=ap&fullName=Mark&checksum=wxyz
- http&#58;//yourserver.com/averox/api/join?meetingID=test01&password=ap&fullName=Chris&createTime=273648&checksum=abcd
...
<response>
  ...
  <url>https://yourserver.com/client/Averox.html?sessionToken=ai1wqj8wb6s7rnk0</url>
</response>
```
...
https&#58;//yourserver.com/averox/api/insertDocument?[parameters]&checksum=[checksum]
...
You can do the request either via averoxserver or `curl`, which I am going to demonstrate in the following paragraph.
...
``` bash
curl -s -X POST "https://{your-host}/averox/api/insertDocument?meetingID=Test&checksum=6b76e90b9a20481806a7ef513bc81ef0299609ed" --header "Content-Type: application/xml" --data '{xml}'
```

Combining both together, we get:

``` bash
curl -s -X POST "https://{your-host}/averox/api/insertDocument?meetingID=Test&checksum=6b76e90b9a20481806a7ef513bc81ef0299609ed" --header "Content-Type: application/xml" --data '<modules>
   <module name="presentation">
        <document current="true" downloadable="true" url="{link to download the presentation}" filename="sample.pdf"/>
        <document removable="false" name="sample.pdf">
        JVBERi0xLjQKJdPr6eEKMSAwIG9iago8PC9UaXRsZSAoT3RoZXJfc2FtcGxlKQovUHJvZHVjZXIgKFNraWEvUERGIG0xMDAgR29vZ2xlIERvY3MgUmVuZGVyZXIpPj4KZW5... {base64 encoded document}
...
http&#58;//yourserver.com/averox/api/isMeetingRunning?[parameters]&checksum=[checksum]
...
http&#58;//yourserver.com/averox/api/isMeetingRunning?meetingID=test01&checksum=1234
...
- http&#58;//yourserver.com/averox/api/end?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/end?meetingID=1234567890&password=mp&checksum=1234
...
```bash
curl --request POST \
	--url https://<your-host>/averox/api/end \
	--header 'Content-Type: application/x-www-form-urlencoded' \
...
- http&#58;//yourserver.com/averox/api/getMeetingInfo?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/getMeetingInfo?meetingID=test01&checksum=1234
...
<response>
	...
	<metadata>
		<avx-origin-version>summit2024-6d8120x</avx-origin-version>
		<avx-origin-server-name>test30.averox.org</avx-origin-server-name>
		<avx-recording-ready-url>https://test30.averox.org/recording_ready</avx-recording-ready-url>
		<avx-origin>averoxserver</avx-origin>
		<endcallbackurl>https://test30.averox.org/meeting_ended</endcallbackurl>
	</metadata>
...
http&#58;//yourserver.com/averox/api/getMeetings?checksum=[checksum]

**Parameters:**

Since Averox 0.80, it is no more required to pass any parameter for this call.

**Example Requests:**

http&#58;//yourserver.com/averox/api/getMeetings?checksum=1234
...
http&#58;//yourserver.com/averox/api/getRecordings?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/getRecordings?checksum=1234
- http&#58;//yourserver.com/averox/api/getRecordings?meetingID=CS101&checksum=abcd
- http&#58;//yourserver.com/averox/api/getRecordings?meetingID=CS101,CS102&checksum=wxyz
- http&#58;//yourserver.com/averox/api/getRecordings?recordID=652c9eb4c07ad49283554c76301d68770326bd93-1462283509434&checksum=wxyz
- http&#58;//yourserver.com/averox/api/getRecordings?recordID=652c9eb4c07ad49283554c76301d68770326bd93-1462283509434,9e359d17635e163c4388281567601d7fecf29df8-1461882579628&checksum=wxyz
- http&#58;//yourserver.com/averox/api/getRecordings?recordID=652c9eb4c07ad49283554c76301d68770326bd93&checksum=wxyz
- http&#58;//yourserver.com/averox/api/getRecordings?recordID=652c9eb4c07ad49283554c76301d68770326bd93,9e359d17635e163c4388281567601d7fecf29df8&checksum=wxyz
- http&#58;//yourserver.com/averox/api/getRecordings?state=published&offset=20&limit=10&checksum=abc123
   ...
   <recordings>
      <recording>
         ...
         <playback>
            <format>
               <type>podcast</type>
               <url>https://demo.averox.org/podcast/ffbfc4cc24428694e8b53a4e144f414052431693-1530718721124/audio.ogg</url>
               <processingTime>0</processingTime>
               <length>0</length>
            </format>
            <format>
               <type>presentation</type>
               <url>https://demo.averox.org/playback/presentation/2.0/playback.html?meetingId=ffbfc4cc24428694e8b53a4e144f414052431693-1530718721124</url>
               <processingTime>7177</processingTime>
               <length>0</length>
               <preview>
                  <images>
                     <image alt="Welcome to" height="136" width="176">https://demo.averox.org/presentation/ffbfc4cc24428694e8b53a4e144f414052431693-1530718721124/presentation/d2d9a672040fbde2a47a10bf6c37b6a4b5ae187f-1530718721134/thumbnails/thumb-1.png</image>
                     <image alt="(this slide left blank for use as a whiteboard)" height="136" width="176">https://demo.averox.org/presentation/ffbfc4cc24428694e8b53a4e144f414052431693-1530718721124/presentation/d2d9a672040fbde2a47a10bf6c37b6a4b5ae187f-1530718721134/thumbnails/thumb-2.png</image>
                     <image alt="(this slide left blank for use as a whiteboard)" height="136" width="176">https://demo.averox.org/presentation/ffbfc4cc24428694e8b53a4e144f414052431693-1530718721124/presentation/d2d9a672040fbde2a47a10bf6c37b6a4b5ae187f-1530718721134/thumbnails/thumb-3.png</image>
                  </images>
               </preview>
            </format>
         </playback>
      ...
      <recording>
         ...
         <playback>
            <format>
               <type>podcast</type>
               <url>https://demo.averox.org/podcast/ffbfc4cc24428694e8b53a4e144f414052431693-1530278898111/audio.ogg</url>
               <processingTime>0</processingTime>
               <length>33</length>
            </format>
            <format>
               <type>presentation</type>
               <url>https://demo.averox.org/playback/presentation/2.0/playback.html?meetingId=ffbfc4cc24428694e8b53a4e144f414052431693-1530278898111</url>
               <processingTime>139458</processingTime>
               <length>33</length>
               <preview>
                  <images>
                     <image width="176" height="136" alt="Welcome to">https://demo.averox.org/presentation/ffbfc4cc24428694e8b53a4e144f414052431693-1530278898111/presentation/d2d9a672040fbde2a47a10bf6c37b6a4b5ae187f-1530278898120/thumbnails/thumb-1.png</image>
                     <image width="176" height="136" alt="(this slide left blank for use as a whiteboard)">https://demo.averox.org/presentation/ffbfc4cc24428694e8b53a4e144f414052431693-1530278898111/presentation/d2d9a672040fbde2a47a10bf6c37b6a4b5ae187f-1530278898120/thumbnails/thumb-2.png</image>
                     <image width="176" height="136" alt="(this slide left blank for use as a whiteboard)">https://demo.averox.org/presentation/ffbfc4cc24428694e8b53a4e144f414052431693-1530278898111/presentation/d2d9a672040fbde2a47a10bf6c37b6a4b5ae187f-1530278898120/thumbnails/thumb-3.png</image>
                  </images>
               </preview>
            </format>
         </playback>
...
- http&#58;//yourserver.com/averox/api/publishRecordings?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/publishRecordings?recordID=record123&publish=true&checksum=1234
- http&#58;//yourserver.com/averox/api/publishRecordings?recordID=record123,recordABC&publish=true&checksum=wxyz
...
http&#58;//yourserver.com/averox/api/deleteRecordings?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/deleteRecordings?recordID=record123&checksum=1234
- http&#58;//yourserver.com/averox/api/deleteRecordings?recordID=record123,recordABC&checksum=wxyz
...
- http&#58;//yourserver.com/averox/api/updateRecordings?[parameters]&checksum=[checksum]
...
- http&#58;//yourserver.com/averox/api/updateRecordings?recordID=record123&meta_Presenter=Jane%20Doe,meta_category=FINANCE,meta_TERM=Fall2016&checksum=1234
...
`GET http://yourserver.com/averox/api/getRecordingTextTracks?[parameters]&checksum=[checksum]`
...
- live - A caption track derived from live captioning performed in a Averox.
...
Errors
: In addition to the standard Averox checksum error, this API call can return the following errors in `<messageKey>` when returncode is FAILED:
...
Note that the standard Averox checksum algorithm must be performed on the GET parameters, but that the body of the request (the subtitle file) is not checksummed.
...
`POST http://yourserver.com/averox/api/putRecordingTextTrack`
...
http&#58;//yourserver.com/averox/api/sendChatMessage?[parameters]&checksum=[checksum]
...
http&#58;//yourserver.com/averox/api/sendChatMessage?meetingID=test01&message=my+test&userName=System+Admin&checksum=1234
...
Averox provides API Sample Codes so you can integrated easily with your application. Feel free to contribute and post your implementation of the API in other language code in the averox-dev mailing list.

### PHP

There is stable PHP library in [Averox PHP API](https://github.com/averox/averox-api-php).
...
For more examples of using PHP, see the source for the [Moodle](https://github.com/blindsidenetworks/moodle-mod_averoxbn) and [WordPress](https://github.com/blindsidenetworks/wordpress-plugin_averox) integrations.

### Ruby

See the following [averox-api-ruby](https://github.com/mconf/averox-api-ruby) gem created by the good folks at [Mconf](http://mconf.org).

### Testing API Calls with API Mate

To help you create/test valid API calls against your Averox server, use the excellent [API Mate](http://mconf.github.io/api-mate/) to interactively create API calls. API Mate generates the checksums within the browser (no server component needed) so you can use it to test API calls against a local Averox server.
...
If your using API Mate to test recordings and want to query by `meetingID`, be sure to clear the `recordID` field first.  Averox's API supports querying for recordings by either value, but not both at the same time.
...
When major events happen within meetings, it would be very helpful to provide a way for third-party applications to be notified of these events. For instance, when a user joins a conference, they will presumably be joining through the third-party app. However, when they leave the conference, the app may have certain auditing that it needs to do to record their disconnect time, etc. If Averox could make some callback to the application, this would assist in such scenarios.

For example, the application may be able to register a URL that Averox would call with status updates. Averox would define an API that such an app would be required to implement at that URL. For example, Averox could call:

- http&#58;//third-party-app/avx-integ.php?event=meetingEnded&meetingID=abcd
- http&#58;//third-party-app/avx-integ.php?event=userLeft&userID=1234
- http&#58;//third-party-app/avx-integ.php?event=meetingEnded&meetingID=abcd
...
All these valid formats are also present in a list in the [back-end](https://github.com/averox/averox/blob/v2.7.10/avx-common-web/src/main/java/org/averox/presentation/MimeTypeUtils.java#L28-L47) and in the [front-end](https://github.com/averox/averox/blob/v2.7.10/averox-html5/private/config/settings.yml#L824-L862) if more details are needed.