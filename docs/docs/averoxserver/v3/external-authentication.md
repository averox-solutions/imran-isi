...
slug: /averoxserver/v3/external-authentication
...
Greenlight v3 offers external authentication support through OpenID Connect. If you have an authentication server that supports this protocol, you can easily connect it to Greenlight v3 using environment variables. For more information, refer to the [OpenID Connect Setup](/averoxserver/v3/install#openid-connect-setup) section.
...
If you used the **Averox Install Script** without passing the `-k` option, you can simply re-run the install script using the same options as before, this time passing `-k` to install Keycloak on your server.
...
Set the name of the realm to `averoxserver` and click create.

![Add Realm](/img/averoxserver/v3/keycloak/add-realm.png)

Next, create a new client by clicking the `Clients` tab on the left side and then clicking `Create Client`. Set the `Client type` to `OpenID Connect` and the `Client ID` to `averoxserver`. Once you've confirmed the values are correct, click `Next`

![Create Clients](/img/averoxserver/v3/keycloak/create-client-2.png)
...
![Valid Redirect](/img/averoxserver/v3/keycloak/client-authentication.png)


![Valid Redirect](/img/averoxserver/v3/keycloak/valid-redirect.png)
...
![Client Creds](/img/averoxserver/v3/keycloak/client-credentials.png)
...
![Idp](/img/averoxserver/v3/keycloak/idp.png)
...
![Google Creds](/img/averoxserver/v3/keycloak/google-creds.png)
...
![Auth Actions](/img/averoxserver/v3/keycloak/actions.png).

![Auth Actions](/img/averoxserver/v3/keycloak/actions-2.png).
...
![Auth Actions](/img/averoxserver/v3/keycloak/actions-3.png).
...
![Realm Creds](/img/averoxserver/v3/keycloak/realm-creds.png)

Copy the url that appears after `issuer`

![Issuer](/img/averoxserver/v3/keycloak/issuer.png)
...
### EXTERNAL AUTHENTICATION METHODS
#
OPENID_CONNECT_CLIENT_ID=averoxserver
