...
const joinEndpointTableData = [
  ...
  },
  {
    ...
    "required": false,
    "type": "String",
    "description": (<>Third-party apps using the API can now pass createTime parameter (which was created in the create call), Averox will ensure it matches the ‘createTime’ for the session.  If they differ, Averox will not proceed with the join request. This prevents a user from reusing their join URL for a subsequent session with the same meetingID.</>)
  },
  {
    "name": "userID",
    "required": false,
  ...
  },
  {
    ...
    "required": false,
    "type": "String",
    "description": (<>If you want to pass in a custom voice-extension when a user joins the voice conference using voip. This is useful if you want to collect more info in you Call Detail Records about the user joining the conference. You need to modify your /etc/asterisk/avx-extensions.conf to handle this new extensions.</>)
  },
  {
    "name": "avatarURL",
    "required": false,
    "type": "String",
    "description": (<>The link for the user’s avatar to be displayed (default can be enabled/disabled and set with “useDefaultAvatar“ and “defaultAvatarURL“ in avx-web.properties).</>)
  },
  {
    "name": "redirect",
    "required": false,
