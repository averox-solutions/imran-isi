...
const createEndpointTableData = [
  {
    ...
    "required": true,
    "type": "String",
    "description": (<>A name for the meeting.  This is now required as of Averox 2.4.</>)
  },
  {
    "name": "meetingID",
    "required": true,
  ...
  },
  {
    ...
    "required": false,
    "type": "String",
    "description": (<>The URL that the Averox client will go to after users click the OK button on the ‘You have been logged out message’.  This overrides the value for <code className="language-plaintext highlighter-rouge">averox.web.logoutURL</code> in <a href="https://github.com/averox/averox/blob/master/averox-web/grails-app/conf/averox.properties">averox.properties</a>.</>)
  },
  {
    "name": "record",
    "required": false,
    "type": "Boolean",
    "description": (<>Setting ‘record=true’ instructs the Averox server to record the media and events in the session for later playback. The default is false.<br /><br /> In order for a playback file to be generated, a moderator must click the Start/Stop Recording button at least once during the session; otherwise, in the absence of any recording marks, the record and playback scripts will not generate a playback file. See also the <code className="language-plaintext highlighter-rouge">autoStartRecording</code> and <code className="language-plaintext highlighter-rouge">allowStartStopRecording</code> parameters in <a href="https://github.com/averox/averox/blob/master/averox-web/grails-app/conf/averox.properties">averox.properties</a>.</>)
  },
  {
    "name": "duration",
    "required": false,
    "type": "Number",
    "description": (<>The maximum length (in minutes) for the meeting.<br /><br /> Normally, the Averox server will end the meeting when either (a) the last person leaves (it takes a minute or two for the server to clear the meeting from memory) or when the server receives an <a href="#end">end</a> API request with the associated meetingID (everyone is kicked and the meeting is immediately cleared from memory).<br /><br /> Averox begins tracking the length of a meeting when it is created.  If duration contains a non-zero value, then when the length of the meeting exceeds the duration value the server will immediately end the meeting (equivalent to receiving an end API request at that moment).</>)
  },
  {
    "name": "isBreakout",
    "required": false,
  ...
  },
  {
    ...
    "type": "Boolean",
    "deprecated": true,
    "description": (<>Removed in 2.3 in favor of <code className="language-plaintext highlighter-rouge">meetingKeepEvents</code> and averox.properties <code className="language-plaintext highlighter-rouge">defaultKeepEvents</code>.</>)
  },
  {
    "name": "meetingKeepEvents",
    "type": "Boolean",
    "default": "false",
    "description": (<>Defaults to the value of <code className="language-plaintext highlighter-rouge">defaultKeepEvents</code>. If <code className="language-plaintext highlighter-rouge">meetingKeepEvents</code> is true Averox saves meeting events even if the meeting is not recorded (added in 2.3)</>)
  },
  {
  ...
  },
  {
    ...
    "type": "Boolean",
    "default": "true",
    "description": (<><b>[DEPRECATED]</b> Removed in 2.5, temporarily still handled, please transition to disabledFeatures.<br /><br />Default <code className="language-plaintext highlighter-rouge">learningDashboardEnabled=true</code>. When this option is enabled Averox generates a Dashboard where moderators can view a summary of the activities of the meeting. (added 2.4)</>)
  },
  {
  ...
  },
  {
    ...
    "required": false,
    "type": "String",
    "description": (<>Pass a URL to an image which will then be visible in the area above the participants list if <code>displayBrandingArea</code> is set to <code>true</code> in avx-html5's configuration</>)
  },
  {
