#!/usr/bin/python3
#
# Script to setup a GNS3 project for testing Averox.
#
...
# Averox servers see the dummy public subnet as their public IP
# address.
#
# smallstep.com's step-ca server is used to run an ACME Certificate
# Authority in a manner compatible with certbot, allowing avx-install
...
#
# Averox servers with NAT gateways will be configured to forward
...
# (default "Averox").  This node also operates the step-ca and
...
# USAGE
#
# ./gns3-avx.py focal-260    (to install a released server)
# ./gns3-avx.py focal-e41349 (to install a server based on a git commit)
# ./gns3-avx.py testclient   (to install a test client)
#
# By default, the GNS3 project is called "Averox".  If the
...
parser = argparse.ArgumentParser(parents=[gns3.parser('Averox')],
                                 description='Start an Averox test network in GNS3',
...
parser.add_argument('-r', '--repository', type=str,
                    help='package repository to be used for Averox server install')
parser.add_argument('-g', '--averoxserver', action='store_true',
                    help='install Greenlight')
parser.add_argument('--ubuntu-release', type=int, default=20,
                    help='Ubuntu release (18 or 20; default 20) to be used for Averox server install')
parser.add_argument('--release', type=str,
                    help='Averox release to be used for Averox server install (default is server hostname)')
parser.add_argument('--install-script', type=str,
                    help='install script to be used for Averox server install\n'
                    + 'can be a local file, a URL, or a filename on https://ubuntu.averox.org/')
parser.add_argument('--proxy-server', type=str,
                    help='proxy server to be passed to Averox server install script')
parser.add_argument('--no-nat', action='store_true',
                    help='install BBB server without a NAT gateway')
parser.add_argument('--no-install', action='store_true',
                    help="don't run avx-install script on BBB server")
...
parser.add_argument('version', nargs='*',
                    help="""version of Averox server to be installed
...
ssl_root_key_fn = os.path.join(__location__, 'avx-dev-ca.key')
ssl_root_crt_fn = os.path.join(__location__, 'avx-dev-ca.crt')
try:
...
def master_gateway(hostname, public_subnet=None, x=0, y=0):
    ...
    # resolver1.opendns.com is used by avx-install to determine external IP address
...
"""
    ...
    ca = {
	"root": "/opt/ca/avx-dev-ca.crt",
	"crt": "/opt/ca/avx-dev-ca.crt",
	"key": "/opt/ca/avx-dev-ca.key",
	"address": ":8000",
...
"""
    ...
    user_data = {'hostname': hostname,
                 ...
                 'write_files': [
                     ...
                     {'path': '/opt/ca/avx-dev-ca.key',
                      'permissions': '0400',
                      'content': ssl_root_key
                     },
                     {'path': '/opt/ca/avx-dev-ca.crt',
                      'permissions': '0444',
                      'content': ssl_root_crt
                     },
                     {'path': '/usr/local/share/ca-certificates/avx-dev/avx-dev-ca.crt',
                      'permissions': '0444',
                      'content': ssl_root_crt
                     },
                     # /var/www/html/avx-dev-ca.crt should no longer be used for anything
                     {'path': '/var/www/html/avx-dev-ca.crt',
                      'permissions': '0444',
                      'content': ssl_root_crt
                     },
                 ],
...
def turn_server(hostname, x=0, y=0):
    ...
    if not args.no_install:
        user_data['runcmd'].append(f'wget -qO- https://ubuntu.averox.org/avx-install.sh | sudo bash -s -- -c {hostname}.{args.domain}:secret -e root@{hostname}.{args.domain}')
...
# A NAT gateway for some kind of server (either Averox or natturn)
...
"""
    ...
    user_data = {'hostname': hostname + '-NAT',
                 ...
                 'write_files': [
                     ...
                     # Our NAT configuration tunnels port 22 through to the Averox server
                 ...
                 'runcmd': [
                     ...
                     # Also, avx-install requires the server to have the ability to connect to itself using its DNS name,
...
def BBB_server_standalone(hostname, x=100, y=300):
    ...
    # A Averox server without an associated NAT gateway
    ...
    if not args.install_script:
        if '25' in args.release:
            args.install_script = 'avx-install-2.5.sh'
        elif '26' in args.release:
            args.install_script = 'avx-install-2.6.sh'
        else:
    ...
    if args.install_script.startswith('http:') or args.install_script.startswith('https:'):
        ...
        install_script_request = requests.get(f"https://ubuntu.averox.org/{args.install_script}")
        install_script_request.raise_for_status()
        install_script = install_script_request.text
    ...
    if not args.no_install:
        ...
        if args.repository:
            ...
            repo = 'ubuntu.averox.org'

        distro = ubuntu_release[args.ubuntu_release]
        try:
            requests.head(f'https://{repo}/{args.release}/dists/averox-{distro}/Release.gpg').raise_for_status()
...
# The Averox servers and/or test clients

for v in args.version:
    if v == 'turn':
        # A standard Averox TURN server
        ...
        # A Averox TURN server behind a NAT gateway (like AWS or Azure)
