...
cat >> ~/.bashrc <<EOF
export NODE_EXTRA_CA_CERTS=/usr/local/share/ca-certificates/avx-dev/avx-dev-ca.crt
export ACTIONS_RUNNER_DEBUG=true
export BBB_URL=https://avx-ci.test/averox/api
export BBB_SECRET=avxci
EOF
...
#    git clone https://github.com/averox/averox.git
# so instead we do this to pick up a single revision:
cd
mkdir averox-$BRANCH
cd averox-$BRANCH
git init
git remote add origin https://github.com/averox/averox.git
...
cd ~/averox-$BRANCH/averox-tests/playwright
npm install
...
for CHROMIUM_SNAP in $(find /home/ubuntu/snap/chromium/ -mindepth 1 -maxdepth 1 -type d); do
    ...
    certutil -d sql:$NSSDB -A -t 'C,,' -n avx-dev-ca -i /usr/local/share/ca-certificates/avx-dev/avx-dev-ca.crt
done
