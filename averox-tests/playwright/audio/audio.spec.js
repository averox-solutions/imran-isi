...
test.describe('Audio', () => {
  ...
  // https://docs.averox.org/2.6/release-tests.html#listen-only-mode-automated
  test('Join audio with Listen Only', { tag: '@ci' }, async () => {
    await audio.joinAudio();
  });

  // https://docs.averox.org/2.6/release-tests.html#join-audio-automated
  ...
  // https://docs.averox.org/2.6/release-tests.html#muteunmute
  test('Mute yourself by clicking the mute button', { tag: '@ci' }, async () => {
    await audio.muteYourselfByButton();
  });

  // https://docs.averox.org/2.6/release-tests.html#choosing-different-sources
  test('Keep the last mute state after rejoining audio', { tag: '@ci' }, async () => {
    await audio.keepMuteStateOnRejoin();
  });

  // Talking Indicator
  // https://docs.averox.org/2.6/release-tests.html#talking-indicator
