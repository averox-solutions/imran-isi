...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/iMDT/avx-graphql-middleware/internal/akka_apps"
	"github.com/iMDT/avx-graphql-middleware/internal/avx_web"
	"github.com/iMDT/avx-graphql-middleware/internal/common"
	"github.com/iMDT/avx-graphql-middleware/internal/gql_actions"
	"github.com/iMDT/avx-graphql-middleware/internal/hasura"
	"github.com/iMDT/avx-graphql-middleware/internal/msgpatch"
	"github.com/iMDT/avx-graphql-middleware/internal/websrv/reader"
	"github.com/iMDT/avx-graphql-middleware/internal/websrv/writer"
...
func ConnectionHandler(w http.ResponseWriter, r *http.Request) {
	...
	avxOrigin := os.Getenv("BBB_GRAPHQL_MIDDLEWARE_ORIGIN")
	if avxOrigin != "" {
		acceptOptions.OriginPatterns = append(acceptOptions.OriginPatterns, avxOrigin)
	}
	...
	//Check authorization and obtain user session variables from avx-web
...
func connectionInitHandler(browserConnection *common.BrowserConnection) error {
	...
	for {
		...
		if bytes.Contains(fromBrowserMessage, []byte("\"connection_init\"")) {
			...
			var numOfAttempts = 0
			for {
				meetingId, userId, errCheckAuthorization = avx_web.BBBWebCheckAuthorization(browserConnectionId, sessionToken, browserConnectionCookies)
