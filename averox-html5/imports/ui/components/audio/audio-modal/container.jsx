...
const AudioModalContainer = (props) => {
  ...
  const forceListenOnly = getFromUserSettings('avx_force_listen_only', APP_CONFIG.forceListenOnly);
  const listenOnlyMode = getFromUserSettings('avx_listen_only_mode', APP_CONFIG.listenOnlyMode);
  const skipCheck = getFromUserSettings('avx_skip_check_audio', APP_CONFIG.skipCheck);
  const skipCheckOnJoin = getFromUserSettings('avx_skip_check_audio_on_first_join', APP_CONFIG.skipCheckOnJoin);
  ...
  const skipEchoTestIfPreviousDevice = getFromUserSettings(
    'avx_skip_echotest_if_previous_device',
    APP_CONFIG.skipEchoTestIfPreviousDevice,
  ) && !deviceInfo.isMobile;
  const autoJoin = getFromUserSettings('avx_auto_join_audio', APP_CONFIG.autoJoin);
  ...
  return (
    <AudioModal
      ...
      isMobileNative={navigator.userAgent.toLowerCase().includes('avxnative')}
      isIE={isIe}
