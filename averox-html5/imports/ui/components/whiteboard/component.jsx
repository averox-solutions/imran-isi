...
import "@averox/tldraw/tldraw.css";
...
const Whiteboard = React.memo(function Whiteboard(props) {
  ...
  React.useEffect (() => {
    const avx_multi_user_tools = getFromUserSettings(
      'avx_multi_user_tools',
      ...
      if (avx_multi_user_tools.length >= 1 && !isModerator) {
        allElements.forEach((element) => {
          const toolName = element.getAttribute('data-testid').split('.')[1];

          if (!avx_multi_user_tools.includes(toolName)) {
    ...
    React.useEffect (() => {
      const avx_presenter_tools = getFromUserSettings(
        'avx_presenter_tools',
        meetingClientSettingsInitialValues.public.whiteboard.toolbar.presenterTools,
      );
        const allElements = document.querySelectorAll('[data-testid^="tools."]');

        if (avx_presenter_tools.length >= 1 && isPresenter) {
          allElements.forEach((element) => {
            const toolName = element.getAttribute('data-testid').split('.')[1];

            if (!avx_presenter_tools.includes(toolName)) {
              element.style.display = 'none';
            }
          });
        }
      ...
      React.useEffect (() => {
        const avx_multi_user_pen_only = getFromUserSettings(
          'avx_multi_user_pen_only',
          false,
        );
          const allElements = document.querySelectorAll('[data-testid^="tools."]');

          if (avx_multi_user_pen_only && !isModerator && !isPresenter) {
            allElements.forEach((element) => {
              const toolName = element.getAttribute('data-testid').split('.')[1];
