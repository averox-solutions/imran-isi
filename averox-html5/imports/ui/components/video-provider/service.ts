...
class VideoService {
  ...
  getRecord(myRole?: string) {
    ...
    if (this.record === null) {
      this.record = getFromUserSettings('avx_record_video', true);
    }
  ...
  static mirrorOwnWebcam(userId: string | null = null) {
    ...
    const isEnabledMirroring = getFromUserSettings('avx_mirror_own_webcam', MIRROR_WEBCAM);
