...
import { notifyStreamStateChange } from '/imports/ui/services/avx-webrtc-sfu/stream-state-service';
import VideoPreviewService from '/imports/ui/components/video-preview/service';
import MediaStreamUtils from '/imports/utils/media-stream-utils';
import BBBVideoStream from '/imports/ui/services/webrtc-base/avx-video-stream';
import {
  EFFECT_TYPES,
  getSessionVirtualBackgroundInfo,
} from '/imports/ui/services/virtual-background/service';
import { notify } from '/imports/ui/services/notification';
import { shouldForceRelay } from '/imports/ui/services/avx-webrtc-sfu/utils';
...
class VideoProvider extends Component<VideoProviderProps, VideoProviderState> {
  ...
  destroyWebRTCPeer(stream: string) {
    ...
    if (peer) {
      if (peer && peer.avxVideoStream) {
        if (typeof peer.inactivationHandler === 'function') {
          peer.avxVideoStream.removeListener('inactive', peer.inactivationHandler);
        }
        peer.avxVideoStream.stop();
      }
  ...
  private createPublisher(stream: string, peerOptions: Record<string, unknown>) {
    return new Promise((resolve, reject) => {
      try {
        ...
        let avxVideoStream = VideoService.getPreloadedStream();

        if (avxVideoStream) {
          peerOptions.videoStream = avxVideoStream.mediaStream;
        }

        const peer = new WebRtcPeer('sendonly', peerOptions);
        peer.avxVideoStream = avxVideoStream;
        ...
        peer.generateOffer().then((offer) => {
          ...
          if (peer.avxVideoStream == null) {
            avxVideoStream = new BBBVideoStream(peer.getLocalStream());
            VideoPreviewService.storeStream(
              MediaStreamUtils.extractDeviceIdFromStream(
                avxVideoStream.mediaStream,
                'video',
              ),
              avxVideoStream,
            );
          ...
          peer.avxVideoStream = avxVideoStream;
          avxVideoStream.on('streamSwapped', ({ newStream }) => {
          ...
          avxVideoStream.once('inactive', peer.inactivationHandler);
          resolve(offer);
        }).catch(reject);
      } catch (error) {
        reject(error);
  ...
  startVirtualBackgroundByDrop(stream: string, type: string, name: string, data: string) {
    return new Promise((resolve, reject) => {
      const peer = this.webRtcPeers[stream];
      const { avxVideoStream } = peer;
      const video = this.getVideoElement(stream);

      if (peer && video && video.srcObject) {
        avxVideoStream.startVirtualBackground(type, name, { file: data })
          .then(resolve)
          .catch(reject);
      }
    }).catch((error) => {
