...
class VideoPreview extends Component {
  ...
  set currentVideoStream (avxVideoStream) {
    ...
    if (avxVideoStream) {
      avxVideoStream.once('inactive', this.handleLocalStreamInactive);
    }
    this._currentVideoStream = avxVideoStream;
  }
  ...
  stopVirtualBackground(avxVideoStream) {
    if (avxVideoStream) {
      avxVideoStream.stopVirtualBackground();
      this.displayPreview();
    }
  }

  startVirtualBackground(avxVideoStream, type, name, customParams) {
    this.setState({ isStartSharingDisabled: true });

    if (avxVideoStream == null) return Promise.resolve(false);

    return avxVideoStream.startVirtualBackground(type, name, customParams).then(() => {
      this.displayPreview();
      return true;
    }).catch(error => {
  ...
  async getCameraStream(deviceId, profile) {
    ...
    try {
      ...
      const avxVideoStream = await PreviewService.doGUM(deviceId, profile);
      this.currentVideoStream = avxVideoStream;
    ...
    try {
      ...
      if (!this._isMounted) {
        this.terminateCameraStream(avxVideoStream, deviceId);
        this.cleanupStreamAndVideo();
        return;
      }
      this.setState({
